#!/bin/bash
#
# description: test11b - Full control of entitlement attribute
# configs: ldap-attributes
#
# This pspng config is for pspng to have full control of the subject's provisioned ldap attribute
...
test_step "Checking that xyz:pdq is no longer an attribute value"
x=$(run_in_grouper_daemon bash -c "myldapsearch -b "$user_dn" eduPersonEntitlement | grep_ xyz:pdq")

assert_empty "$x" "Expected xyz:pdq to be removed as an eduPersonEntitlement value"
