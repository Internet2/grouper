...
Grouper plugin that provides configurable authentication. The plugin uses version 6.1.2 of pac4j, a standalone
...
repository, in the grouper-misc/grouper-ext-auth-logic module. The environment includes sample Grouper configurations for SAML2, OIDC, or CAS, along with a Shibboleth IDP
that can authenticate Grouper using any of these methods.

=== Adding the plugin jar to the image

Since Grouper v5.20.6 the plugin is include in the image, in file /opt/grouper/plugins/grouper-authentication-plugin.jar. The version number is left out of
the filename, but the underlying version is the same as the rest of Grouper.

In previous versions, the latest version of the pac4j plugin jar can be downloaded from https://repo1.maven.org/maven2/edu/internet2/middleware/grouper/grouper-ext-auth/[the Maven repository]
...
## Set this to match your SSO lifetime, or <= 0 to not check the authn issueInstant; Note, if value is too low pac4j
## returns a 500 error instead of redirecting to the IDP. Default is 3600
# external.authentication.saml.maximumAuthenticationLifetime = 3600
...
Since it is an independent plugin, the log4j setup used by Grouper is not used. Instead, it is bundled with
https://www.slf4j.org/api/org/slf4j/simple/SimpleLogger.html[slf4j-simple],  a basic but functional logger. Log levels and
formats can be controlled through Java system properties. For example:

----
GROUPER_EXTRA_CATALINA_OPTS=-Dorg.slf4j.simpleLogger.log.org.pac4j=info
----
...
- maximumAuthenticationLifetime (optional, default 3600): Number of seconds to honor the authentication response
issueInstant time. Nominally from shibboleth2.xml, `<Sessions>` timeout attribute. But for pac4j, ideally it should
match the IDP's idp.authn.defaultLifetime (in conf/auth/authn.properties) since pac4j does not correctly redirect to
the IDP on an issueInstant that is too old. Can also set to 0 or below to disable authn instant checking.
