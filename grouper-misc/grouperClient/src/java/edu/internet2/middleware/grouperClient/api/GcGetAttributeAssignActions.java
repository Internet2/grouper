...
public class GcGetAttributeAssignActions {
  ...
  private void validate() {
    if (GrouperClientUtils.length(this.namesOfAttributeDefs) == 0
        && GrouperClientUtils.length(this.uuidsOfAttributeDefs) == 0 &&
        GrouperClientUtils.length(this.idIndexesOfAttributeDefs) == 0) {
      throw new RuntimeException(
          "atleast one of the attributeDefNames, attributeDefUuids and attributeDefIdIndexes "
              + "is required : " + this);
    }
  }
    ...
    try {
...

      {
        ...
        for (Long idIndexOfAttributeDef : this.idIndexesOfAttributeDefs) {
          attributeDefLookups.add(new WsAttributeDefLookup(null, null,
              idIndexOfAttributeDef.toString()));
        }
        if (GrouperClientUtils.length(attributeDefLookups) > 0) {
          getAttributeAssignActions.setWsAttributeDefLookups(GrouperClientUtils.toArray(
              attributeDefLookups, WsAttributeDefLookup.class));
        }
      }

      //add params if there are any
      if (this.params.size() > 0) {
        getAttributeAssignActions.setParams(GrouperClientUtils.toArray(this.params,
            WsParam.class));
      }

      if (GrouperClientUtils.length(this.actions) > 0) {
        getAttributeAssignActions.setActions(GrouperClientUtils.toArray(this.actions,
            String.class));
      }
      ...
      String resultMessage = wsGetAttributeAssignActionsResults.getResultMetadata()
          .getResultMessage();
      grouperClientWs.handleFailure(wsGetAttributeAssignActionsResults, null,
          resultMessage);
