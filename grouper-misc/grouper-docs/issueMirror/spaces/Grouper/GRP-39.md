...
key: GRP-39
cloud_key: 
onprem_url: https://todos.internet2.edu/browse/GRP-39
type: Improvement
status: Resolved
resolution: Fixed
priority: Trivial
...
created: 2007-09-20T16:37:35.258+0000
updated: 2008-01-04T05:44:24.181+0000
resolved: 2007-10-31T14:21:54.759+0000
components: [UI]
fixVersions: [1.2.1]
labels: []
links: []
...
Controlling which person subject attributes display on the subject summary page is simple enough in the sources.xml file, but there is no ability to control the order these attributes appear on the page. This creates a chaotic page display where an orderly page display is needed to support troubleshooting.

## Comments

### James Cramton - 2007-09-20T16:38:48.331+0000

The attributes displayed on the Subject Summary page for a person should be controlled through some mechanism.

### Gary Brown - 2007-09-25T15:48:31.964+0000

Currently I do a Subject.getAttributes().keyset() which is at the mercy of HashMap.keyset(). I would suggest adding some properties to media.properties i.e.

subject.attributes.order.<SOURCE_ID>=comma separated list of attribute names

attributes not listed would be ignored. 

This would require a fairly straightforward change to the Subject Summary action. Would you be happy with this approach?

### James Cramton - 2007-09-25T16:14:57.835+0000

listing a sort order in the media.properties file like that would be a very reasonable approach.

On a related note, I remember that when we switched from an ldap-based person registry to a sql-based registry, the <attribute> tags in the sources.xml file that were required in order to retrieve the ldap attributes are ignored when using a sql source. In the SQL source, the attribute names defined in the SQL source's search query defined the attributes displayed on the subject summary page. However, there are some oddities in how SQL attributes can be named; for example, displayName will not display on the page, presumably because it is defined in the Name_AttributeType and Description_AttributeType in our sources.xml file. But Display Name is displayed on the subject search page. 

I'm wondering if it wouldn't make sense to consolidate the <attribute> tags in the sources.xml file with the list in the media.properties file? I know they serve different purposes, but keeping them separate could introduce room for error.

What we should probably do is to document these issues when we implement the sort order configuration in the media.properties file, since these issues are not well documented, as far as I can tell.

### Gary Brown - 2007-10-31T14:21:54.729+0000

We can come back to the Subject source definitions. 

Order of attributes may be controlled through media.properties:


### Gary Brown - 2007-10-31T14:23:02.353+0000

Keep on doing something which leads to premature submission...
...
### dede - 2008-01-04T05:44:24.090+0000
...
## Attachments
- edu.brown.subjectSummaryUnsortedAttribs.jpg (187940 bytes) - by James Cramton on 2007-09-20T16:38:48.260+0000