...
The join key is the SAML `nameId`. **Institution TODO:** confirm what your IdP asserts to the GitHub enterprise (eppn such as `pennkey@upenn.edu`, or an opaque id); that value must exist as a Grouper subject attribute and is the match column.
...
POST $SERVICE_URL$/orgs/$ORG$/invitations Content-Type: application/json { "email": "someuser@example.edu", "role": "direct_member" } // optional combined team assign: "team_ids": [ 17937664 ] 201 { "id": 77992097, "login": null, "email": "someuser@example.edu", "role": "direct_member", "failed_at": null, "team_count": 0 }
