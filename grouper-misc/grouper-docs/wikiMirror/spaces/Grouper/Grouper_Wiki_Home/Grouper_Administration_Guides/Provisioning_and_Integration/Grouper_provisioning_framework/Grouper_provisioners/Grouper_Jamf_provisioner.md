...
Accounts are matched and created by `name` = lowercased EPPN (`pennkey@example.com`). Because the Classic API has no atomic add/remove for account groups, membership is applied by retrieve-modify-write of the whole member list; only `<name>`/`<members>` are sent, so a membership change never rewrites a role's privileges.
