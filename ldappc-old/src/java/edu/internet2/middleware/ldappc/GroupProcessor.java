...
public class GroupProcessor 
{
    ...
    /**
     * Delete a member
     * @param group the group containing the member to be deleted
    ...
    public boolean deleteMember(Group group, Subject subject)
    {
        ...
        if (subject != null)
        {
            ...
            {
                success = false;
                ErrorLog.error(this.getClass(), "Could not delete member: " 
                        + subject.getName() + " -- " + mde.getMessage()); 
            }
        ...
        {
            ErrorLog.error(this.getClass(), "Attempting to delete a null member with a null subject.");
        }
        return success;
    }
    ...
    public boolean deleteMember(Group group, Member member)
    {
        ...
        if (member != null)
        {
            ...
            catch (MemberDeleteException mde)
            {
                success = false;
                ErrorLog.error(this.getClass(), "Could not delete member: " 
                        + member.toString() + " -- " + mde.getMessage()); 
            }
            catch (InsufficientPrivilegeException ipe) 
            {
                success = false;
            ...
            {
                ErrorLog.error(this.getClass(), "Trying to delete a member that can not be found: " 
                        + snfe.getMessage());
            }
        }
        else
     ...
     * @param group the group to which a new member is to be added
     * @param subject the subject to be added as a member of the group
     * @return true if member was added else false which include not added because
    ...
    public boolean addMember(Group group, Subject subject) 
    {
        ...
        else
        {
            ErrorLog.error(this.getClass(), "Error attempting to to add a member, "
                    + member.toString() + ", to an non-existent group.");
        }
        return added;
    }
} 
