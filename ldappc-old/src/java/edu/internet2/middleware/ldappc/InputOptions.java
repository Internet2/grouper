...
/**
 * Class for processing command line input arguments, to verify them, and to make
 * them or values calculated from them available to the rest of the code.
 * @author Gil Singer 
 */
public class InputOptions implements GrouperProvisionerOptions, SignetProvisionerOptions
{
    ...
    /**
     * Flag for skipping logging of error messages when doing a JUnit test run
     */
     ...
     * @param isTest isTest is true if this is a test case run.
     */
     ...
     * @param doMemberships The doMemberships.
     */
     ...
     * @param doPermissions The doPermissions.
     */
    ...
    /**
     * The doPolling. 
     * An indicator that polling is to be performed.
     * It is set if the -interval parameter is used as has a non-zero value. 
     */
    ...
    /**
     * The configManagerLocation is the location of an alternative
     ...
     * @param configManagerLocation The location of the configuration manager xml file.
     */
     ...
     * The Keys shown below must appear exactly as shown; the values column represents fields that
     * the user inputs as appropriate. 
     *  
     ...
     * -permissions                       (Opt) When present, privilege information will be provisioned
     ...
     * -configManager  configMgr          (Opt) Location of substitute configuration manager file, other than default.
     *
     * If -subject, -lastModifyTime, or -interval are used, then values of the appropriate type must
     * follow them.
     *
     * Value           Type
     * ------------    ------------------- 
     * subjectId       String 
     * lastModifyTime  String of the following format:
     *                 DateTime representation to select only objects changed since then.
     ...
     * interval        int 
     * configMgr       A String representation of the file name for the configuration manager.
     * If no arguments are used, a list of the allowed arguments is displayed to standard out. 
     * If any arguments are used, then -subject subjectId are required. 
     */
    
    /**
     * Constructor: This no argument constructor is needed for testing so that
     * setIsTest can be called before initialization.  Except for the JUnit
     * test cases use the construct that has input arguments.
     */
    public InputOptions()
    {  
    }
    ...
    /**
     * Constructor: verifies the input arguments and makes them accessible through
    ...
    /**
     * This initializer verifies the input arguments and makes them accessible through
     * the use of getter methods.
     */
    public void init(String[] args)
    {
        try
        {
            if (args.length == 0)
            {
                    ...
                    ErrorLog.fatal(this.getClass(), "Failed attempting to start program with no arguments.");
            }
            ...
            //
            // Loop over all input arguments, perform checking, and set program variables from this input.
            //
              
            //
            // Note that for the -subject and -interval and -lastModifyTime, the index i is
            ...
            for (int i=0; i<args.length; i++)
            {
                ...
                if (args[i].startsWith("-"))
                {
                    ...
                    else if (args[i].equalsIgnoreCase("-lastModifyTime"))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //
                                // Change from String to a Date and check String for format.
                                // If short, fill in zeros for hours, minutes and/or seconds.
                                //
                                ...
                                {
                                    ErrorLog.fatal(this.getClass(), 
                                            dateInputAdjusted + " is incorrect format for lastModifyTime.  "
                    ...
                    else if (args[i].equalsIgnoreCase("-interval"))
                    {
                        ...
                        {
                            setDoPolling(true);
                            // Check for reasonable value and if too small, set to MIN_INTERVAL.
                    ...
                    else if (args[i].equalsIgnoreCase("-configManager"))
                    {
                        if (i+1 >= args.length)
                        {
                            ErrorLog.fatal(this.getClass(), "FATAL ERROR, Missing a value for -configManager.");
                            setFatal(true);
                        }
                        else
                        {
                ...
                else
                {
                    if (!isTest())
                    {
                        ErrorLog.warn(this.getClass(), "WARNING, BAD INPUT, Ignoring parameter: arg[" + i + "]=" + args[i]);
                    }
                }
            }
            ...
            // Allow no arguments at all which will produce a console message of allowed values.
        ...
        catch (Exception e)
        {
            if (!isTest())
            {
                ErrorLog.fatal(this.getClass(), "FATAL ERROR, Unexpected Exception Type in InputOptions.");
            }
            setFatal(true);
            e.printStackTrace(); 
        }
