 ...
 */
public class JNDISourceAdapterTest
        extends TestCase {
    /**
     * Source adapter
     */
    ...
    protected void setUp() {
        ...
        source = new JNDISourceAdapter("jndi", "JNDI Subject Source");
        source.addInitParam("INITIAL_CONTEXT_FACTORY",
                "com.sun.jndi.ldap.LdapCtxFactory");
        source.addInitParam("PROVIDER_URL", "ldap://localhost:389");
        source.addInitParam("SECURITY_AUTHENTICATION", "simple");
        source.addInitParam("SECURITY_PRINCIPAL",
                "cn=Manager,dc=example,dc=edu");
        source.addInitParam("SECURITY_CREDENTIALS", "secret");
        source.addInitParam("SubjectID_AttributeType", "kitnEduPersonRegId");
        source.addInitParam("Name_AttributeType", "cn");
        ...
        source.addAttribute("sn");
        source.addAttribute("department");
    ...
    public void testIdSearch() {
        ...
        try {
            subject = source.getSubject("SD00001");
        ...
        try {
            subject = source.getSubject("chris");
            fail("Searching id = chris: null expected but found result");
        } catch (SubjectNotFoundException e) {
            assertTrue("Searching id = chris: null expected and found null",
                    true);
        } catch (SubjectNotUniqueException e) {
    ...
    public void testIdentifierSearch() {
        Subject subject = null;
        try {
            subject = source.getSubjectByIdentifier("comalley");
            assertEquals("Searching dentifier = SD00001", "SD00001", subject
                    .getId());
        } catch (SubjectNotFoundException e) {
        ...
        try {
            subject = source.getSubjectByIdentifier("chris");
            fail("Searching identifier = chris: null expected but found result");
        } catch (SubjectNotFoundException e) {
            assertTrue(
                    "Searching identifier = chris: null expected and null found",
                    true);
        } catch (SubjectNotUniqueException e) {
    ...
    public void testLDAPInjection() {
        ...
        set = source.search("comalley)(description=President");
