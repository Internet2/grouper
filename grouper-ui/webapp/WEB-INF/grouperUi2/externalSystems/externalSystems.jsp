...
${grouper:title('externalSystemsPageTitle')}
  ...
  <div class="page-header blue-gradient">
    ...
    <div class="row-fluid">
      <div class="lead span9 pull-left">
        <h4>${textContainer.text['miscellaneousGrouperExternalSystemsMainDescription'] }</h4>
      </div>
...
<div class="row-fluid">
  ...
  <c:choose>
    <c:when
      test="${fn:length(grouperRequestContainer.externalSystemContainer.guiGrouperExternalSystems) > 0}">
      ...
      <table
        ...
        <tbody>
          <c:set var="i" value="0" />
          <c:forEach
            items="${grouperRequestContainer.externalSystemContainer.guiGrouperExternalSystems}"
            var="guiGrouperExternalSystem">
            ...
            <tr>
              ...
              <td style="white-space: nowrap;"><c:if test="${guiGrouperExternalSystem.grouperExternalSystem.enabled == true}">
                            ${textContainer.text['grouperExternalSystemTableEnabledTrueValue']}
                           </c:if> <c:if test="${guiGrouperExternalSystem.grouperExternalSystem.enabled == false }">
              ...
              <td>
                <div class="btn-group">
                  ...
                  <ul class="dropdown-menu dropdown-menu-right"
                    id="more-options${i}">
                    <c:if
                      test="${guiGrouperExternalSystem.grouperExternalSystem.enabled == true}">
                      <li><a href="#"
                    ...
                    <li><a href="#"
                      onclick="return guiV2link('operation=UiV2ExternalSystem.editExternalSystemConfigDetails&externalSystemConfigId=${guiGrouperExternalSystem.grouperExternalSystem.configId}&externalSystemType=${guiGrouperExternalSystem.grouperExternalSystem['class'].name}');">${textContainer.text['grouperExternalSystemTableEditDetailsActionOption'] }</a></li>
                    <c:if
                      test="${guiGrouperExternalSystem.grouperExternalSystem.enabled == false}">
                      <li><a href="#"
                        onclick="return ajax('../app/UiV2ExternalSystem.enableExternalSystem?externalSystemConfigId=${guiGrouperExternalSystem.grouperExternalSystem.configId}&externalSystemType=${guiGrouperExternalSystem.grouperExternalSystem['class'].name}');">${textContainer.text['grouperExternalSystemTableEnableActionOption'] }</a></li>
                    </c:if>
                    <li><a href="#"
    ...
    <c:otherwise>
      <div class="row-fluid">
        <div class="span9">
          <p>
            <b>${textContainer.text['grouperExternalSystemNoConfiguredExternalSystemsFound'] }</b>
          </p>
        </div>
      </div>
    </c:otherwise>
