              ...
              <form id="attributeDefNamesToDeleteFormId">
                ...
                <table class="table table-hover table-bordered table-striped table-condensed data-table">
                  <thead>
                    <c:if test="${isAdmin}" >
                      <tr>
                        <td colspan="4" class="table-toolbar gradient-background"><button type="button" onclick="ajax('../app/UiV2AttributeDefName.deleteAttributeDefNames?attributeDefId=${grouperRequestContainer.attributeDefContainer.guiAttributeDef.attributeDef.id}', {formIds: 'attributeDefFilterFormId,attributeDefNamesToDeleteFormId', formIdsOptional: 'attributeDefPagingFormId'}); return false;" class="btn">${textContainer.text['attributeDefRemoveSelectedAttributeDefNamesButton'] }</button></td>
                      </tr>
                    </c:if>
                    <tr>
                      <c:if test="${isAdmin}" >
                  ...
                  <tbody>
                    ...
                    <c:forEach items="${grouperRequestContainer.attributeDefContainer.guiAttributeDefNames}" var="guiAttributeDefName">
                      <tr>
                        ...
                        <td>
                          <div class="btn-group">
                          	<button type="button" data-toggle="dropdown" aria-label="${textContainer.text['ariaLabelGuiMoreAttributeNameActions']}" class="btn btn-mini dropdown-toggle" aria-haspopup="true" aria-expanded="false" onclick="$('#attribute-more-options${i}').is(':visible') === true ? $(this).attr('aria-expanded','false') : $(this).attr('aria-expanded',function(index, currentValue) { $('#attribute-more-options${i} li').first().focus();return true;});">
