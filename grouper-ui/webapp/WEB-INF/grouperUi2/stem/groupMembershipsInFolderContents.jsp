                      ...
                      <tr>
                        <td colspan="${grouperRequestContainer.stemContainer.showEnabledStatus ? 9 : 6}" class="table-toolbar gradient-background">
                          <c:if test="${!grouperRequestContainer.stemContainer.showPointInTimeAudit}">
                            <button type="button" onclick="ajax('../app/UiV2Stem.removeGroupMembers?stemId=${grouperRequestContainer.stemContainer.guiStem.stem.id}', {formIds: 'groupFilterFormId,membersToDeleteFormId', formIdsOptional: 'groupPagingFormId'}); return false;" class="btn">${textContainer.text['groupRemoveSelectedMembersButton'] }</button>
                          </c:if>
                        </td>
                      </tr>
                      <tr>
                                ...
                                <div class="btn-group"><button type="button" data-toggle="dropdown" aria-label="${textContainer.text['ariaLabelGuiMoreMembershipActions']}" class="btn btn-mini dropdown-toggle"
                                  ...
                                  <ul class="dropdown-menu dropdown-menu-right" id="more-options${i}">
                                    ...
                                    <c:if test="${guiMembershipContainer.membershipContainer.membershipAssignType.immediate && guiMembershipSubjectContainer.guiGroup.canUpdate}">
                                      <li><button type="button" class="grouper-menuitem" onclick="ajax('../app/UiV2Stem.removeGroupMember?stemId=${grouperRequestContainer.stemContainer.guiStem.stem.id}&groupId=${guiMembershipSubjectContainer.guiGroup.group.id}&memberId=${guiMembershipSubjectContainer.guiMember.member.uuid}', {formIds: 'groupFilterFormId', formIdsOptional: 'groupPagingFormId'}); return false;" class="actions-revoke-membership">${textContainer.text['groupViewRevokeMembershipButton'] }</button></li>
                                    </c:if>
