...
<div id="stemDetailsId">
       ...
       <c:if test="${ (isWheelGroupMember || grouperRequestContainer.stemContainer.canAdminPrivileges) && grouperRequestContainer.stemContainer.guiAttestation.type == 'DIRECT'}">
         <a href="javascript:void(0)" onclick="return guiV2link('operation=UiV2Attestation.editStemAttestation&stemId=${grouperRequestContainer.stemContainer.guiAttestation.attributeAssignable.id}'); return false;" class="btn btn-medium btn-block btn-primary" role="button">${textContainer.text['editAttestationGroupButton'] }</a>
       </c:if>
       <c:if test="${ (isWheelGroupMember || grouperRequestContainer.stemContainer.canAdminPrivileges) && grouperRequestContainer.stemContainer.guiAttestation.type == 'INDIRECT'}">
           ...
           onclick="return guiV2link('operation=UiV2Attestation.addStemAttestation&stemId=${grouperRequestContainer.stemContainer.guiStem.stem.id}'); return false;"
