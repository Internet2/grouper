<%@ include file="../assetsJsp/commonTaglib.jsp"%>
  ...
  <tr>
    ...
    <td>
      ...
      <input type="text" style="width: 30em" value="${grouper:escapeHtml(guiUserLifecycleActionConfig.userLifecycleActionConfiguration.configId)}"
         name="userLifecycleActionConfigId" id="userLifecycleActionConfigId" />
        <span class="requiredField" rel="tooltip" data-html="true" data-delay-show="200" data-placement="right"
        data-original-title="${textContainer.textEscapeDouble['grouperRequiredTooltip']}">*</span>
      
      <br />
      <span class="description">${textContainer.text['userLifecycleActionConfigIdHint']}</span>
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      ...
      onchange="ajax('../app/UiV2UserLifecycle.addUserLifecycleActionConfiguration', {formIds: 'userLifecycleActionsConfigDetails'}); return false;"
      >
       
        <option value=""></option>
        <c:forEach items="${grouperRequestContainer.userLifecycleContainer.allUserLifecycleActionTypes}" var="userLifecycleAction">
          <option value="${userLifecycleAction['class'].name}"
              ${guiUserLifecycleActionConfig.userLifecycleActionConfiguration['class'].name == userLifecycleAction['class'].name ? 'selected="selected"' : '' }
              >${userLifecycleAction.title}</option>
        </c:forEach>
      </select>
      <span class="requiredField" rel="tooltip" data-html="true" data-delay-show="200" data-placement="right" 
      data-original-title="${textContainer.textEscapeDouble['grouperRequiredTooltip']}">*</span>
      <br />
      <span class="description">${textContainer.text['userLifecycleActionTypeHint']}</span>
    </td>
  </tr>
    
  <c:forEach items="${guiUserLifecycleActionConfig.userLifecycleActionConfiguration.subSections}" var="subSection">
      <tbody>
        ...
        <c:forEach items="${subSection.attributesValues}" var="attribute">
          ...
          <grouper:configFormElement 
            ...
            ajaxCallback="ajax('../app/UiV2UserLifecycle.addUserLifecycleActionConfiguration', {formIds: 'userLifecycleActionsConfigDetails'}); return false;"
