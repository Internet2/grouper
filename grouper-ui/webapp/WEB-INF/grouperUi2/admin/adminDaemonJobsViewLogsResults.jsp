                  ...
                  <tbody>
                    ...
                    <c:forEach var="guiHib3GrouperLoaderLog" items="${grouperRequestContainer.adminContainer.guiHib3GrouperLoaderLogs}" >
                      ...
                      <tr>
                        ...
                        <td style="white-space: nowrap"><span class="jobMessageContainer"><grouper:abbreviateTextarea text="${guiHib3GrouperLoaderLog.hib3GrouperLoaderLog.jobMessage}" showCharCount="30" cols="20" rows="3"/></span><c:if test="${not empty guiHib3GrouperLoaderLog.hib3GrouperLoaderLog.jobMessage}"> <button type="button" onclick="grouperCopyJobMessage(this); return false;" aria-label="${textContainer.textEscapeXml['copyToClipboardTooltip']}" title="${textContainer.text['copyToClipboardTooltip']}" class="btn-link" style="margin-left:4px;padding:0;border:0;background:none;vertical-align:baseline;"><i class="fa fa-clone" aria-hidden="true"></i></button></c:if></td>
                        ...
                        <td style="white-space: nowrap">
                          <c:if test="${guiHib3GrouperLoaderLog.hib3GrouperLoaderLog.status == 'ERROR_FAILSAFE' && grouperRequestContainer.adminContainer.guiDaemonJobs.get(0).failsafeNeedsApproval}">
                            <button type="button" class="btn-link" style="padding:0;border:0;background:none;font-family:inherit;font-size:inherit;line-height:inherit;vertical-align:baseline;" onclick="ajax('../app/UiV2Admin.daemonJobsSubmit?action=failsafeApprove&source=logs&jobName=${grouper:escapeUrl(grouperRequestContainer.adminContainer.guiDaemonJobs.get(0).jobName)}'); return false;">${textContainer.text['adminDaemonJobsMoreActionsFailsafeApprove'] }</button>
                          </c:if>
                        </td>
