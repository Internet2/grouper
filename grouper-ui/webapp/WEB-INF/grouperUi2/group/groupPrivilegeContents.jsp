                        ...
                        <div class="row-fluid">
                          ...
                          <div class="span4">
                            <button type="submit" class="btn" 
                              onclick="ajax('../app/UiV2Group.assignPrivilegeBatch?groupId=${grouperRequestContainer.groupContainer.guiGroup.group.id}', {formIds: 'groupFilterPrivilegesFormId,groupPrivilegeFormId', formIdsOptional: 'groupPagingPrivilegesFormId,groupPagingPrivilegesFormPageNumberId'}); return false;">${textContainer.text['groupPrivilegesUpdateSelected'] }</button>
                          </div>
                        </div>
                      </td>
                    </tr>
                      ...
                      <tr>
                        ...
                        <c:forEach items="admins,readers,updaters,optins,optouts,groupAttrReaders,groupAttrUpdaters,viewers" var="fieldName">
                          <td data-hide="phone,medium" class="direct-actions privilege" >
                            ...
                            <%-- GRP-7096: build specific accessible names for this cell, e.g. "Remove Update for jsmith" (column label minus its line break) --%>
                            <c:choose>
                            ...
                            <c:choose>
                              <c:when test="${guiMembershipContainer != null 
                                ...
                                <button type="button" class="privilege-check-btn fa fa-check fa-direct" title="${grouper:escapeHtml(ariaRemovePrivilege)}" aria-label="${grouper:escapeHtml(ariaRemovePrivilege)}" onclick="if (confirmChange('${textContainer.textEscapeSingleDouble['groupConfirmChanges']}')) {ajax('../app/UiV2Group.assignPrivilege?assign=false&groupId=${grouperRequestContainer.groupContainer.guiGroup.group.id}&fieldName=${fieldName}&memberId=${guiMembershipSubjectContainer.guiMember.member.uuid}', {formIds: 'groupFilterPrivilegesFormId', formIdsOptional: 'groupPagingPrivilegesFormId,groupPagingPrivilegesFormPageNumberId'});} return false;"></button>
                                <button type="button" title="${grouper:escapeHtml(ariaRemovePrivilege)}" aria-label="${grouper:escapeHtml(ariaRemovePrivilege)}" class="btn btn-inverse btn-super-mini remove"
                                   onclick="if (confirmChange('${textContainer.textEscapeSingleDouble['groupConfirmChanges']}')) {ajax('../app/UiV2Group.assignPrivilege?assign=false&groupId=${grouperRequestContainer.groupContainer.guiGroup.group.id}&fieldName=${fieldName}&memberId=${guiMembershipSubjectContainer.guiMember.member.uuid}', {formIds: 'groupFilterPrivilegesFormId', formIdsOptional: 'groupPagingPrivilegesFormId,groupPagingPrivilegesFormPageNumberId'});} return false;"
                                  ><i class="fa fa-times" aria-hidden="true"></i></button>
                              </c:when>
                              <c:otherwise>
                                <c:if test="${guiMembershipContainer != null}"><button type="button" class="privilege-check-btn fa fa-check fa-disabled" title="${grouper:escapeHtml(ariaAssignPrivilege)}" aria-label="${grouper:escapeHtml(ariaAssignPrivilege)}" onclick="if (confirmChange('${textContainer.textEscapeSingleDouble['groupConfirmChanges']}')) {ajax('../app/UiV2Group.assignPrivilege?assign=true&groupId=${grouperRequestContainer.groupContainer.guiGroup.group.id}&fieldName=${fieldName}&memberId=${guiMembershipSubjectContainer.guiMember.member.uuid}', {formIds: 'groupFilterPrivilegesFormId', formIdsOptional: 'groupPagingPrivilegesFormId,groupPagingPrivilegesFormPageNumberId'});} return false;"></button></c:if>
