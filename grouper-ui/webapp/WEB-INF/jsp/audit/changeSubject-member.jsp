...
--%><%--
  @author Gary Brown.
  @version $Id: changeSubject-member.jsp,v 1.2 2009-08-10 14:03:01 isgwb Exp $
--%>
...
<grouper:message bundle="${nav}" key="audit.result.label.for"/><br/>
<c:choose>
	...
	<c:set var="idChangedMessage"><grouper:message bundle="${nav}" key="audit.result.label.member-id-did-not-change"/></c:set>
	</c:when>
	<c:otherwise>
	<c:set var="subject" value="${viewObject.fieldObjects.newMemberId}"/>
	<c:set var="memberIdName">newMemberId</c:set>
	<c:set var="idChangedMessage"><grouper:message bundle="${nav}" key="audit.result.label.member-id-did-change"/></c:set>
	</c:otherwise>
</c:choose>
...
<c:choose>
    ...
    <tiles:insert definition="dynamicTileDef" flush="false">
		...
		<c:otherwise>
		<grouper:message bundle="${nav}" key="audit.result.label.unavailable"/> 
