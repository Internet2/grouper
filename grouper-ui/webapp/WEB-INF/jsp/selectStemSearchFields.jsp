...
--%><%--
  @author Gary Brown.
  @version $Id: selectStemSearchFields.jsp,v 1.5 2009-09-09 15:10:03 mchyzer Exp $
--%>
	...
	<tr class="formTableRow">
		...
		<td class="formTableRight">
			...
			<select name="<c:out value="${searchFieldAndOrNot}"/>">
				<option value="or" <c:if test="${advancedSearchStemFieldParams[searchFieldAndOrNot]=='or'}">selected="selected"</c:if>><grouper:message key="find.search.or"/></option>
				<option value="and"  <c:if test="${advancedSearchStemFieldParams[searchFieldAndOrNot]=='and'}">selected="selected"</c:if>><grouper:message key="find.search.and"/></option>
				<option value="not" <c:if test="${advancedSearchStemFieldParams[searchFieldAndOrNot]=='not'}">selected="selected"</c:if>><grouper:message key="find.search.not"/></option>
			</select>
			</c:if>
		</td>
	</tr>
