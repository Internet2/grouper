...
--%><%--
  @author Gary Brown.
  @version $Id: groupSearchResultWithPrivsView.jsp,v 1.1 2007-09-30 08:58:17 isgwb Exp $
--%>
...
<html:link page="/populateGroupMember.do" name="params" title="${navMap['groups.access.modify-all.title']}">
  ...
  <c:forTokens var="priv" items="MEMBER ADMIN UPDATE READ VIEW OPTIN OPTOUT" delims=" ">
	...
	<c:if test="${hasPriv != 'false'}">
		<span class="has<c:out value="${hasPriv}"/>PrivForGroup"><fmt:message bundle="${nav}" key="${priv}"/></span>
	</c:if>
	</td>
  </c:forTokens>
  </tr>
