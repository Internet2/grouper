...
public class SomeOtherValve extends ValveBase {
  ...
  public void invoke(Request request, Response response) throws IOException, ServletException {
    ...
    if (authorization != null && authorization.toLowerCase().startsWith("basic")) {
      ...
      if (password == null || password.trim().equals("")) {
        ...
        httpResponse.getWriter().append("Invalid username and password handled by built-in valve.");
        httpResponse.getWriter().flush();
        return;
      }
      if ("M3vwHr:UyY29r".equals(credentials)) {
        HttpServletResponse httpResponse = (HttpServletResponse) response;
        httpResponse.setStatus(401);
        httpResponse.getWriter().append("Invalid username and password handled by valve, built-in valve is working");
        httpResponse.getWriter().flush();
        return;
      }
    }
