...
public class TierServiceProviderProviderIntegrationTest {
  ...
  public void getServiceProviderConfig() throws IOException {
    ...
    assertThat(json.get("baseUrn").asText(), equalTo("urn:ietf:params:scim:schemas:core:2.0:ServiceProviderConfig"));
    assertThat(json.get("changePassword").get("supported").asBoolean(), equalTo(Boolean.FALSE));
    /* todo PSU impl not sure it supports patch */
    assertThat(json.get("patch").get("supported").asBoolean(), equalTo(Boolean.TRUE));
    assertThat(json.get("filter").get("maxResults").asInt(), equalTo(100));
    assertThat(json.get("filter").get("supported").asBoolean(), equalTo(Boolean.FALSE));
    /* todo PSU has non-standard extensions object */
    assertThat(json.get("extensions").asText(), equalTo(""));
    ...
    /* todo PSU impl not sure it supports bulk */
    assertThat(json.get("bulk").get("supported").asBoolean(), equalTo(Boolean.TRUE));
    assertThat(json.get("bulk").get("supported").asBoolean(), equalTo(Boolean.TRUE));
    assertThat(json.get("bulk").get("maxPayloadSize").asInt(), greaterThan(0));
    assertThat(json.get("bulk").get("maxOperations").asInt(), greaterThan(0));
  }
}
