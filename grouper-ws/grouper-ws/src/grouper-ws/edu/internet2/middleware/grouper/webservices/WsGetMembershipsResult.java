...
public class WsGetMembershipsResult {
	 ...
	 */
	public WsGetMembershipsResult(Membership membership,
			boolean retrieveExtendedSubjectDataBoolean) {
		this.setMembershipId(membership.getUuid());
		this.setMembershipType(membership.getType());
		this.setCreateTime(GrouperServiceUtils.dateToString(membership
				.getCreateTime()));
		...
		FieldType listFieldType = listField == null ? null : listField
				.getType();
		this.setListType(listFieldType == null ? null : listFieldType
				.toString());
		...
		if (member != null) {
			...
			if (retrieveExtendedSubjectDataBoolean) {
				...
				this.setSubjectType(theSubjectType == null ? null
						: theSubjectType.getName());
...

				{
					...
					if (!StringUtils.isBlank(attributeName0)
							&& attributeName0
									.startsWith(WsGetMembersResult.SUBJECT_ATTRIBUTE_PREFIX)) {
						attributeName0 = attributeName0
								.substring(WsGetMembersResult.SUBJECT_ATTRIBUTE_PREFIX
										.length());
						this.setAttribute0(subject
								.getAttributeValue(attributeName0));
					}
				}

				{
					...
					if (!StringUtils.isBlank(attributeName1)
						...
						attributeName1 = attributeName1
								.substring(WsGetMembersResult.SUBJECT_ATTRIBUTE_PREFIX
										.length());
						this.setAttribute1(subject
								.getAttributeValue(attributeName1));
					}
				}

				{
					...
					if (!StringUtils.isBlank(attributeName2)
						...
						attributeName2 = attributeName2
								.substring(WsGetMembersResult.SUBJECT_ATTRIBUTE_PREFIX
										.length());
						this.setAttribute2(subject
								.getAttributeValue(attributeName2));
					}
				}
			}
		}
	...
	/**
	 * subject name, or group name, or application name returned, this is
	 * extended subject data
	 */
