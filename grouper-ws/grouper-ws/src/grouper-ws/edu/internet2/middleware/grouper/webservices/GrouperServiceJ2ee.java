...
public class GrouperServiceJ2ee implements Filter {
	...
	public static Subject retrieveSubjectLoggedIn() {
		...
		try {
			...
			throw new RuntimeException("Cant find subject from login id: "
					+ userIdLoggedIn, e);
		}
		return caller;
	}
	...
	public static Subject retrieveSubjectActAs(WsSubjectLookup actAsLookup) {
		...
		try {
			...
			throw new RuntimeException(
					"A web service is specifying an actAsUser, but the group specified in "
							+ GrouperWsConfig.WS_ACT_AS_GROUP
							+ " in the grouper-ws.properties: '"
							+ actAsGroupName
							+ "' does not have the logged in user as a member: '"
							+ loggedInSubject + "'");
		} catch (SessionException se) {
			throw new RuntimeException(se);
		} catch (SubjectNotFoundException snfe) {
			throw new RuntimeException("Cant find subject 'GrouperSystem'",
					snfe);
		} catch (SubjectNotUniqueException snue) {
			throw new RuntimeException("Subject 'GrouperSystem' is not unique",
					snue);
			...
			throw new RuntimeException(
					"A web service is specifying an actAsUser, but the group specified in "
							+ GrouperWsConfig.WS_ACT_AS_GROUP
							+ " in the grouper.properties cant be found: "
							+ actAsGroupName, gnfe);
