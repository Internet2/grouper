...
public class GrouperService {
	...
	public WsGetMembersResults getMembers(WsGroupLookup wsGroupLookup,
		...
		try {
			...
			String theError = "Problem getting members from group: wsGroupLookup: " + wsGroupLookup
			...
			//this is sent back to the caller anyway, so just log, and not send back again
			LOG.error(theError + ", wsGetMemberResults: " + GrouperServiceUtils.toStringForLog(wsGetMembersResults), re);
		} finally {
			if (session != null) {
				try {
					session.stop();
	...
	public WsGetMembershipsResults getMemberships(WsGroupLookup wsGroupLookup,
		...
		try {
			...
			String theError = "Problem getting memberships from group: wsGroupLookup: " + wsGroupLookup
			 	+ ", memberFilter: " + membershipFilter + ", retrieveExtendedSubjectData: " 
			 	+ retrieveExtendedSubjectData +  ", actAsSubject: " + actAsSubject
				+ ".  ";
			wsGetMembershipsResults.appendResultMessage(theError + "\n" + ExceptionUtils.getFullStackTrace(re) + ".  ");
			//this is sent back to the caller anyway, so just log, and not send back again
			LOG.error(theError + ", wsGetMembershipResults: " + GrouperServiceUtils.toStringForLog(wsGetMembershipsResults), re);
		} finally {
			if (session != null) {
				try {
					session.stop();
	 ...
	 * @param subjectLookup 
	 * @param subjectLookups subjects to be examined to see if in group
	 * @param memberFilter can be All, Effective (non immediate), Immediate (direct), Composite (if 
	 * composite group with group math (union, minus, etc)
	...
	public WsGetGroupsResults getGroups(
		...
		if (wsMemberFilter == null || !StringUtils.isBlank(parseWsFilterError)) {
			wsGetGroupsResults.assignResultCode(WsGetGroupsResultsCode.INVALID_QUERY);
			wsGetGroupsResults.appendResultMessage("membershipFilter is required and must be valid: '" 
					+ memberFilter + "'\n" + parseWsFilterError);
		...
		if (subject == null) {
			//see why not
			SubjectFindResult subjectFindResult = subjectLookup.retrieveSubjectFindResult();
			String error = "Subject: " + subjectLookup + " had problems: " + subjectFindResult;
			...
			if (subjectFindResult == SubjectFindResult.INVALID_QUERY) {
				...
				wsGetGroupsResults.assignResultCode(WsGetGroupsResultsCode.SUBJECT_NOT_FOUND);
			} else if (subjectFindResult == SubjectFindResult.SUBJECT_DUPLICATE) {
				wsGetGroupsResults.assignResultCode(WsGetGroupsResultsCode.SUBJECT_DUPLICATE);
			} else {
				wsGetGroupsResults.assignResultCode(WsGetGroupsResultsCode.EXCEPTION);
			}
			return wsGetGroupsResults;
		} 
		//TODO make sure size of params and values the same
		...
		try {
			...
			try {
				...
				if (member == null) {
					throw new NullPointerException("Member is null after findBySubject");
				}
			} catch (MemberNotFoundException mvfe) {
				String error = "Subject: " + subjectLookup + " had problems";
				wsGetGroupsResults.setResultMessage(error);
				wsGetGroupsResults.assignResultCode(WsGetGroupsResultsCode.MEMBER_NOT_FOUND);
				return wsGetGroupsResults;
			}
			...
			for (Group group : groups) {
				WsGetGroupsResult wsGetGroupsResult = new WsGetGroupsResult(group);
			...
			//this is sent back to the caller anyway, so just log, and not send back again
			LOG.error(theError + ", wsGetGroupsResults: " + GrouperServiceUtils.toStringForLog(wsGetGroupsResults), re);
		} finally {
			if (session != null) {
				try {
					session.stop();
	 ...
	 * @param wsGroupLookup for the group to see if the members are in there
	 * @param subjectLookups subjects to be examined to see if in group
	 * @param memberFilter can be All, Effective (non immediate), Immediate (direct), Composite (if 
	 * composite group with group math (union, minus, etc)
	 * @param actAsSubjectLookup to act as a different user than the logged in user
	 * @param fieldName is if the Group.hasMember() method with field is to be called
	...
	public WsHasMemberResults hasMember(WsGroupLookup wsGroupLookup,
		...
		if (subjectLength == 0) {
			wsHasMemberResults.assignResultCode(WsHasMemberResultsCode.INVALID_QUERY);
			wsHasMemberResults.appendResultMessage("Subject length must be more than 1");
			return wsHasMemberResults;
		}
		
		//see if greater than the max (or default)
		int maxHasMember = GrouperWsConfig.getPropertyInt(GrouperWsConfig.WS_HAS_MEMBER_SUBJECTS_MAX, 1000000);
		if (subjectLength > maxHasMember) {
			wsHasMemberResults.assignResultCode(WsHasMemberResultsCode.INVALID_QUERY);
			wsHasMemberResults.appendResultMessage("Subject length must be less than max: " + maxHasMember + " (sent in " + subjectLength + ")");
			return wsHasMemberResults;
		}
		...
		if (wsMemberFilter == null || !StringUtils.isBlank(parseWsFilterError)) {
			wsHasMemberResults.assignResultCode(WsHasMemberResultsCode.INVALID_QUERY);
			wsHasMemberResults.appendResultMessage("membershipFilter is required and must be valid: '" 
					+ memberFilter + "'\n" + parseWsFilterError);
			return wsHasMemberResults;
		}
		...
		if (!StringUtils.isBlank(fieldName)) {
			try {
				...
				wsHasMemberResults.assignResultCode(WsHasMemberResultsCode.INVALID_QUERY);
				wsHasMemberResults.appendResultMessage("problems retrieving field: '" + fieldName + "'\n"
		...
		try {
			...
			if (group == null) {
				wsHasMemberResults.assignResultCode(WsHasMemberResultsCode.INVALID_QUERY);
				wsHasMemberResults.appendResultMessage("Cant find group: " + wsGroupLookup + ".  ");
				return wsHasMemberResults;
			}
			...
			for (WsSubjectLookup wsSubjectLookup : subjectLookups) {
				...
				try {
					...
					wsHasMemberResult.setSubjectId(wsSubjectLookup.getSubjectId());
					wsHasMemberResult.setSubjectIdentifier(wsSubjectLookup.getSubjectIdentifier());
					...
					if (subject == null) {
						...
						if (subjectFindResult == SubjectFindResult.SUBJECT_NOT_FOUND) {
							wsHasMemberResult.assignResultCode(WsHasMemberResultCode.MEMBER_NOT_FOUND);
						} else {
							//something bad happened
							wsHasMemberResult.assignResultCode(WsHasMemberResultCode.EXCEPTION);
						}
						continue;
					} 
					...
					boolean hasMember = field == null ? wsMemberFilter.hasMember(group, subject)
							: wsMemberFilter.hasMember(group, subject, field);
					...
					wsHasMemberResult.setResultMessage(ExceptionUtils.getFullStackTrace(e));
			...
			wsHasMemberResults.assignResultCode(WsHasMemberResultsCode.EXCEPTION);
			String theError = "Problem querying member from group: wsGroupLookup: " + wsGroupLookup
				+ ", subjectLookups: " + GrouperServiceUtils.toStringForLog(subjectLookups)
				+ ", actAsSubject: " + actAsSubject
				 + ".  ";
			wsHasMemberResults.appendResultMessage(theError);
			//this is sent back to the caller anyway, so just log, and not send back again
			LOG.error(theError + ", wsHasMemberResults: " + GrouperServiceUtils.toStringForLog(wsHasMemberResults), re);
		} finally {
			if (session != null) {
				try {
					session.stop();
		...
		if (wsHasMemberResults.getResults() != null) {
			...
			for (WsHasMemberResult wsHasMemberResult : wsHasMemberResults.getResults()) {
				boolean success = "T".equalsIgnoreCase(wsHasMemberResult.getSuccess());
				if (success) {
					successes++;
				} else {
					failures++;
			...
			if (failures > 0) {
				wsHasMemberResults.appendResultMessage("There were " + successes + " successes and " + failures 
						+ " failures of users queried hasMember from the group.   ");
				wsHasMemberResults.assignResultCode(WsHasMemberResultsCode.PROBLEM_WITH_QUERY);
			} else {
				wsHasMemberResults.assignResultCode(WsHasMemberResultsCode.SUCCESS);
			}
		}
	 ...
	 * @param groupName to delete the group (mutually exclusive with groupUuid)
	 * @param groupUuid to delete the group (mutually exclusive with groupName)
	 * @param actAsSubjectId optional: is the subject id of subject to act as (if proxying).
	 * Only pass one of actAsSubjectId or actAsSubjectIdentifer
	 * @param actAsSubjectIdentifier optional: is the subject identifier of subject
	 * to act as (if proxying).  Only pass one of actAsSubjectId or actAsSubjectIdentifer
	...
	public WsGroupDeleteResult groupDeleteSimple(String groupName,
		...
		WsGroupDeleteResults wsGroupDeleteResults = groupDelete(wsGroupLookups, actAsSubjectLookup, 
				paramNames, paramValues);
		...
		wsGroupDeleteResult.setResultMessage(wsGroupDeleteResults.getResultMessage());
	
		//convert the outer code to the inner code
		WsGroupDeleteResultsCode wsGroupDeleteResultsCode = wsGroupDeleteResults.retrieveResultCode();
		wsGroupDeleteResult.assignResultCode(wsGroupDeleteResultsCode == null ?
				WsGroupDeleteResultCode.EXCEPTION : wsGroupDeleteResultsCode.convertToResultCode());
	 ...
	 * @param groupName to delete the group (mutually exclusive with groupUuid)
	 * @param groupUuid to delete the group (mutually exclusive with groupName)
	 * @param description of the group, empty will be ignored
	 * @param displayExtension display name of the group, empty will be ignored
	 * @param retrieveViaNameIfNoUuid if to retrieve with name if uuid isnt specified, defaults to true
	 * @param createGroupIfNotExist if the group should be created if it doesnt exist
	 * @param createStemsIfNotExist if the stems should be created if not exist
	 * @param actAsSubjectId optional: is the subject id of subject to act as (if proxying).
	 * Only pass one of actAsSubjectId or actAsSubjectIdentifer
	 * @param actAsSubjectIdentifier optional: is the subject identifier of subject
	 * to act as (if proxying).  Only pass one of actAsSubjectId or actAsSubjectIdentifer
	...
	public WsGroupSaveResult groupSaveSimple(String groupName,
			...
			String retrieveViaNameIfNoUuid,
		    String createGroupIfNotExist, 
		    String createStemsIfNotExist,
			String actAsSubjectId,
			String actAsSubjectIdentifier,
			String paramName0,
			String paramValue0,
		...
		WsGroupToSave wsGroupToSave = new WsGroupToSave(groupUuid, description, 
				displayExtension, retrieveViaNameIfNoUuid, createGroupIfNotExist, 
		...
		WsGroupSaveResults wsGroupSaveResults = groupSave(wsGroupsToSave, actAsSubjectLookup, 
				paramNames, paramValues);
		...
		wsGroupSaveResult.setResultMessage(wsGroupSaveResults.getResultMessage());

		//convert the outer code to the inner code
		WsGroupSaveResultsCode wsGroupSaveResultsCode = wsGroupSaveResults.retrieveResultCode();
		wsGroupSaveResult.assignResultCode(wsGroupSaveResultsCode == null ?
				WsGroupSaveResultCode.EXCEPTION : wsGroupSaveResultsCode.convertToResultCode());
	 ...
	 * @see {@link Group#saveGroup(GrouperSession, String, String, String, String, boolean, boolean, boolean)}
	 * @param wsGroupsToSave groups to save
	 * @param actAsSubjectLookup 
	 * @param paramNames optional: reserved for future use
	 * @param paramValues optional: reserved for future use
	 * @return the results
	...
	public WsGroupSaveResults groupSave(WsGroupToSave[] wsGroupsToSave,
		...
		//see if greater than the max (or default)
		int maxSaveGroup = GrouperWsConfig.getPropertyInt(GrouperWsConfig.WS_GROUP_SAVE_MAX, 1000000);
		if (groupsSize > maxSaveGroup) {
			wsGroupSaveResults.assignResultCode(WsGroupSaveResultsCode.INVALID_QUERY);
			wsGroupSaveResults.appendResultMessage("Number of groups must be less than max: " + maxSaveGroup 
					+ " (sent in " + groupsSize + ")");
		...
		try {
			...
			for (WsGroupToSave wsGroupToSave : wsGroupsToSave) {
				...
				try {
					...
					try {
						...
						wsGroupSaveResult.setResultMessage(ExceptionUtils.getFullStackTrace(e));
						resultIndex++;
						continue;
					}
					...
					wsGroupSaveResult.setResultMessage("Group '" + group.getName() + "' was saved.");
				} catch (InsufficientPrivilegeException ipe) {
					wsGroupSaveResult.assignResultCode(WsGroupSaveResultCode.INSUFFICIENT_PRIVILEGES);
					wsGroupSaveResult.setResultMessage("Error: insufficient privileges to save group '" 
							+ wsGroupToSave.getGroupName() + "'");
				} catch (StemNotFoundException snfe) {
					wsGroupSaveResult.assignResultCode(WsGroupSaveResultCode.STEM_NOT_FOUND);
					wsGroupSaveResult.setResultMessage("Error: stem not found to save group '" 
							+ wsGroupToSave.getGroupName() + "'");
				} catch (GroupNotFoundException gnfe) {
					wsGroupSaveResult.assignResultCode(WsGroupSaveResultCode.GROUP_NOT_FOUND);
					wsGroupSaveResult.setResultMessage("Error: group not found to save group '" 
							+ wsGroupToSave.getGroupName() + "'");
				} catch (Exception e) {
			...
			String theError = "Problem saving groups: wsGroupsToSave: " + GrouperServiceUtils.toStringForLog(wsGroupsToSave)
				+  ", actAsSubject: " + actAsSubject
				 + ".  \n" + "";
			wsGroupSaveResults.appendResultMessage(theError);
			//this is sent back to the caller anyway, so just log, and not send back again
			LOG.error(theError + ", wsGroupSaveResults: " + GrouperServiceUtils.toStringForLog(wsGroupSaveResults), re);
		} finally {
			if (session != null) {
				try {
					session.stop();
		...
		if (wsGroupSaveResults.getResults() != null) {
			...
			for (WsGroupSaveResult wsGroupSaveResult : wsGroupSaveResults.getResults()) {
				boolean success = "T".equalsIgnoreCase(wsGroupSaveResult == null ? null : wsGroupSaveResult.getSuccess());
				if (success) {
					successes++;
				} else {
					failures++;
			...
			if (failures > 0) {
				wsGroupSaveResults.appendResultMessage("There were " + successes 
						+ " successes and " + failures 
						+ " failures of saving groups.   ");
				wsGroupSaveResults.assignResultCode(WsGroupSaveResultsCode.PROBLEM_DELETING_GROUPS);
			} else {
				wsGroupSaveResults.assignResultCode(WsGroupSaveResultsCode.SUCCESS);
			}
		}
	...
	/**
	 * delete a group or many (if doesnt exist, ignore)
	 * @param wsGroupLookups groups to delete
	 * @param actAsSubjectLookup 
	 * @param paramNames optional: reserved for future use
	 * @param paramValues optional: reserved for future use
	 * @return the results
	...
	public WsGroupDeleteResults groupDelete(WsGroupLookup[] wsGroupLookups,
		...
		//see if greater than the max (or default)
		int maxDeleteGroup = GrouperWsConfig.getPropertyInt(GrouperWsConfig.WS_GROUP_DELETE_MAX, 1000000);
		if (groupsSize > maxDeleteGroup) {
			wsGroupDeleteResults.assignResultCode(WsGroupDeleteResultsCode.INVALID_QUERY);
			wsGroupDeleteResults.appendResultMessage("Number of groups must be less than max: " + maxDeleteGroup 
					+ " (sent in " + groupsSize + ")");
		...
		try {
			...
			for (WsGroupLookup wsGroupLookup : wsGroupLookups) {
				...
				try {
					...
					wsGroupDeleteResult.setGroupName(wsGroupLookup.getGroupName());
					wsGroupDeleteResult.setGroupUuid(wsGroupLookup.getUuid());
	
					if (group == null) {
						wsGroupDeleteResult.assignResultCode(WsGroupDeleteResultCode.GROUP_NOT_FOUND);
						wsGroupDeleteResult.setResultMessage("Cant find group: '" + wsGroupLookup + "'.  ");
						continue;
					}
					...
					try {
						...
						wsGroupDeleteResult.setResultMessage("Group '" + group.getName() + "' was deleted.");
					} catch (InsufficientPrivilegeException ipe) {
						wsGroupDeleteResult.assignResultCode(WsGroupDeleteResultCode.INSUFFICIENT_PRIVILEGES);
						wsGroupDeleteResult.setResultMessage("Error: insufficient privileges to delete group '" + group.getName() + "'");
					}
				} catch (Exception e) {
					wsGroupDeleteResult.setResultCode("EXCEPTION");
					wsGroupDeleteResult.setResultMessage(ExceptionUtils.getFullStackTrace(e));
			...
			wsGroupDeleteResults.assignResultCode(WsGroupDeleteResultsCode.EXCEPTION);
			String theError = "Problem deleting groups: wsGroupLookups: " + GrouperServiceUtils.toStringForLog(wsGroupLookups)
				+  ", actAsSubject: " + actAsSubject
				 + ".  \n" + "";
			wsGroupDeleteResults.appendResultMessage(theError);
			//this is sent back to the caller anyway, so just log, and not send back again
			LOG.error(theError + ", wsGroupDeleteResults: " + GrouperServiceUtils.toStringForLog(wsGroupDeleteResults), re);
		} finally {
			if (session != null) {
				try {
					session.stop();
		...
		if (wsGroupDeleteResults.getResults() != null) {
			...
			for (WsGroupDeleteResult wsGroupDeleteResult : wsGroupDeleteResults.getResults()) {
				boolean success = "T".equalsIgnoreCase(wsGroupDeleteResult.getSuccess());
				if (success) {
					successes++;
				} else {
					failures++;
			...
			if (failures > 0) {
				wsGroupDeleteResults.appendResultMessage("There were " + successes 
						+ " successes and " + failures 
						+ " failures of deleting groups.   ");
				wsGroupDeleteResults.assignResultCode(WsGroupDeleteResultsCode.PROBLEM_DELETING_GROUPS);
			} else {
				wsGroupDeleteResults.assignResultCode(WsGroupDeleteResultsCode.SUCCESS);
			}
		}
	...
	public WsAddMemberResults addMember(WsGroupLookup wsGroupLookup,
		...
		int subjectLength = subjectLookups == null ? 0 : subjectLookups.length;
		if (subjectLength == 0) {
			wsAddMemberResults.assignResultCode(WsAddMemberResultsCode.INVALID_QUERY);
				...
				for (Member member : members) {
					...
					if (!newSubjectIds.contains(subjectId)) {
						try {
							...
							wsAddMemberResults.assignResultCode(WsAddMemberResultsCode.PROBLEM_DELETING_MEMBERS);
						}
					}
				}
			}
		} catch (RuntimeException re) {
			wsAddMemberResults.assignResultCode(WsAddMemberResultsCode.EXCEPTION);
		...
		if (wsAddMemberResults.getResults() != null) {
			...
			if (failures > 0) {
				...
				wsAddMemberResults.assignResultCode(WsAddMemberResultsCode.PROBLEM_WITH_ASSIGNMENT);
			} else {
				wsAddMemberResults.assignResultCode(WsAddMemberResultsCode.SUCCESS);
			}
		}
	...
	public WsDeleteMemberResults deleteMember(WsGroupLookup wsGroupLookup,
		...
		//see if greater than the max (or default)
		int maxDeleteMember = GrouperWsConfig.getPropertyInt(GrouperWsConfig.WS_DELETE_MEMBER_SUBJECTS_MAX, 1000000);
		...
		if (wsDeleteMemberResults.getResults() != null) {
			...
			for (WsDeleteMemberResult wsDeleteMemberResult : wsDeleteMemberResults.getResults()) {
				boolean success = "T".equalsIgnoreCase(wsDeleteMemberResult.getSuccess());
				if (success) {
					successes++;
				} else {
					failures++;
	 ...
	 * @param subjectId to add (mutually exclusive with subjectIdentifier)
	 * @param subjectIdentifier to add (mutually exclusive with subjectId)
	 * @param memberFilter can be All, Effective (non immediate), Immediate (direct), Composite (if 
	 * composite group with group math (union, minus, etc)
	 * @param actAsSubjectId optional: is the subject id of subject to act as (if proxying).
	 * Only pass one of actAsSubjectId or actAsSubjectIdentifer
	 ...
	 * @return the result of one member add
	 */
	public WsGetGroupsResults getGroupsSimple(
			String subjectId, 
		...
		WsSubjectLookup subjectLookup = new WsSubjectLookup(subjectId, subjectIdentifier);
		WsSubjectLookup actAsSubjectLookup = new WsSubjectLookup(actAsSubjectId, actAsSubjectIdentifier);
		...
		WsGetGroupsResults wsGetGroupsResults = getGroups(subjectLookup,memberFilter,
	...
	public WsAddMemberResult addMemberSimple(String groupName,
		...
		//convert the outer code to the inner code
		WsAddMemberResultsCode wsAddMemberResultsCode = wsAddMemberResults.retrieveResultCode();
		wsAddMemberResult.assignResultCode(wsAddMemberResultsCode == null ?
				WsAddMemberResultCode.EXCEPTION : wsAddMemberResultsCode.convertToResultCode());
	 ...
	 * @param groupName to lookup the group (mutually exclusive with groupUuid)
	 * @param groupUuid to lookup the group (mutually exclusive with groupName)
	 * @param subjectId to query (mutually exclusive with subjectIdentifier)
	 * @param subjectIdentifier to query (mutually exclusive with subjectId)
	 * @param memberFilter can be All, Effective (non immediate), Immediate (direct), Composite (if 
	 * composite group with group math (union, minus, etc)
	 * @param actAsSubjectId optional: is the subject id of subject to act as (if proxying).
	 * Only pass one of actAsSubjectId or actAsSubjectIdentifer
	...
	public WsHasMemberResult hasMemberSimple(String groupName,
		...
		//didnt even get that far to where there is a subject result
		WsHasMemberResult wsHasMemberResult = new WsHasMemberResult();
		wsHasMemberResult.setResultMessage(wsHasMemberResults.getResultMessage());
		
		//convert the outer code to the inner code
		WsHasMemberResultsCode wsHasMemberResultsCode = wsHasMemberResults.retrieveResultCode();
		wsHasMemberResult.assignResultCode(wsHasMemberResultsCode == null ?
				WsHasMemberResultCode.EXCEPTION : wsHasMemberResultsCode.convertToResultCode());
