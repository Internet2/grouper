...
public class TierGroupProviderIntegrationTest {
  ...
  public void getGroupByUuidInPath() throws IOException {
    ...
    assertThat(json.get("externalId").asText(), equalTo(existingGroupWithNoMembers.getName()));
    ...
    assertThat(json.get("meta").get("resourceType").asText(), equalTo("Group"));
    assertNotNull(json.get("meta").get("version").asText());

    /* currently not used in unboundid version (the dates are return as Long) */
    //assertThat(json.get("meta").get("created"), equalTo(NullNode.getInstance()));
    //assertThat(json.get("meta").get("lastModified"), equalTo(NullNode.getInstance()));
    //assertThat(json.get("meta").get("location"), equalTo(NullNode.getInstance()));
    ...
    assertThat(json.get("urn:grouper:params:scim:schemas:extension:TierGroupExtension").get("systemName").asText(), equalTo("test:existingGroupWithNoMembers"));
    assertThat(json.get("urn:grouper:params:scim:schemas:extension:TierGroupExtension").get("description").asText(), equalTo("Lorem Ipsum 1"));
    assertThat(json.get("urn:grouper:params:scim:schemas:extension:TierGroupExtension").get("idIndex").asLong(), equalTo(existingGroupWithNoMembers.getIdIndex()));
    ...
    // (not standard?) assertThat(json.get("members").get(0).get("type").asText(), equalTo("DIRECT"));
    assertThat(json.get("members").get(0).get("value").asText(), isOneOf(member01.getId(), member02.getId()));
    assertThat(json.get("members").get(0).get("display").asText(), isOneOf(member01.getName(), member02.getName()));
  }
  ...
  public void getGroupFailsWhenNotFoundInPath() throws IOException {
    ...
    assertThat(json.get("detail").asText(), equalTo("edu.internet2.middleware.grouper.exception.GroupNotFoundException: Cant find group by uuid: bogusGroupUuid"));
  ...
  public void getGroupFailsNonNumericIdIndexInPath() throws IOException {
    ...
    // should this be 400 BAD REQUEST instead of 404? PSU returned 404 so this mirrors that legacy result
    assertThat(httpResponse.getStatusLine().getStatusCode(), equalTo(HttpStatus.SC_NOT_FOUND));
    String jsonString = EntityUtils.toString(httpResponse.getEntity());
    ObjectMapper objectMapper = new ObjectMapper();
    JsonNode json = objectMapper.readTree(jsonString);
    assertThat(json.get("detail").asText(), equalTo("java.lang.NumberFormatException: For input string: \"bogusIdIndex\""));
  }
  ...
  public void getGroupFailsWhenNameMissingColonInPath() throws IOException {
    ...
    // should this be 400 BAD REQUEST instead of 404? PSU returns 404
    assertThat(httpResponse.getStatusLine().getStatusCode(), equalTo(HttpStatus.SC_NOT_FOUND));
    String jsonString = EntityUtils.toString(httpResponse.getEntity());
    ObjectMapper objectMapper = new ObjectMapper();
    JsonNode json = objectMapper.readTree(jsonString);
    assertThat(json.get("detail").asText(), equalTo("edu.internet2.middleware.grouper.exception.GroupNotFoundException: Cannot find group with name: 'bogusName'"));
  }
  ...
  public void createGroup() throws IOException {
    ...
    node.putIfAbsent("urn:grouper:params:scim:schemas:extension:TierGroupExtension", tierGroupNode);
    ...
    String jsonString = EntityUtils.toString(httpResponse.getEntity());
    JsonNode json = objectMapper.readTree(jsonString);
    assertThat(json.get("urn:tier:params:scim:schemas:extension:TierMetaExtension").get("resultCode").asText(), equalTo("SUCCESS_CREATED"));
    assertThat(json.get("displayName").asText(), equalTo("test:createGroup"));

    assertThat(json.get("urn:grouper:params:scim:schemas:extension:TierGroupExtension").get("description").asText(), equalTo("createGroup - description"));

    assertThat(json.get("members").size(), equalTo(1));
    assertThat(json.get("members").get(0).get("$ref").asText(), equalTo("../Users/00001"));
    assertThat(json.get("members").get(0).get("display").asText(), equalTo("Subject 01"));
  ...
  public void createGroupFailsOnRootFolder() throws IOException {
    ...
    String jsonString = EntityUtils.toString(httpResponse.getEntity());
    JsonNode json = objectMapper.readTree(jsonString);
    assertThat(json.get("detail").asText(), equalTo("java.lang.IllegalArgumentException: name must contain at least one colon (:)"));
  }
  ...
  public void updateGroup() throws IOException {
    ...
    node.putObject("urn:grouper:params:scim:schemas:extension:TierGroupExtension")
            // Group display name was missing from the TierGroupExtension in the PSU SCIM implementation
    ...
    String jsonString = EntityUtils.toString(httpResponse.getEntity());
    JsonNode json = objectMapper.readTree(jsonString);
    assertThat(json.get("urn:tier:params:scim:schemas:extension:TierMetaExtension").get("resultCode").asText(), equalTo("SUCCESS_UPDATED"));
    ...
    assertThat(json.get("members").get(0).get("display").asText(), equalTo("Subject 02"));
    assertThat(json.get("members").get(0).get("value").asText(), equalTo(member02.getId()));

    assertThat(json.get("urn:grouper:params:scim:schemas:extension:TierGroupExtension").get("description").asText(), equalTo("description changed"));
  }
