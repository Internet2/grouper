...
public class GroupProcessor
{
    ...
    public boolean deleteMember(Group group, Subject subject)
    {
        ...
        if (subject != null)
        {
            ...
            {
                success = false;
                ErrorLog.error(this.getClass(), "Insufficent privilege for deleting member: " + subject.getName()
                        + " -- " + ipe.getMessage());
            }
        }
        else
    ...
    public boolean deleteMember(Group group, Member member)
    {
        ...
        if (member != null)
        {
            ...
            {
                ErrorLog
                        .error(this.getClass(), "Trying to delete a member that can not be found: " + snfe.getMessage());
            }
        }
        else
        {
     ...
     * @return true if member was added else false which include not added
     *         because member already exists.
     */
    public boolean addMember(Group group, Subject subject)
    {
        ...
        else if (group != null)
        {
            ...
            {
                added = false;
                ErrorLog.error(this.getClass(), "Member not added for subject: " + subject.getName() + "\n"
                        + mae.getMessage());
            }
            catch (InsufficientPrivilegeException ipe)
            {
