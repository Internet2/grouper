...
/**
 * Class for processing command line input arguments, to verify them, and to
 * make them or values calculated from them available to the rest of the code.
 * 
 * @author Gil Singer
 */
public class InputOptions implements GrouperProvisionerOptions, SignetProvisionerOptions
{
    ...
    /**
     * Make the lower limit of the frequency of polling a reasonable number of
     * seconds.
     */
     ...
     * This is initialized to false by default.
     */
    ...
    /**
     * The doPolling. An indicator that polling is to be performed. It is set if
     * the -interval parameter is used as has a non-zero value.
     */
    ...
    /**
     * The configManagerLocation is the location of an alternative configuration
     * manager.
     */
     ...
     * setIsTest can be called before initialization. Except for the JUnit test
     * cases use the construct that has input arguments.
     */
    public InputOptions()
    {
    }
    ...
    /**
     * This initializer verifies the input arguments and makes them accessible
     * through the use of getter methods.
     * 
     * @param args
     *            May be any of the following in any order, where (Opt) implies
     *            optional; however, when a key of -xxx is followed by a name, a
     *            value must be present following the key. The Keys shown below
     *            must appear exactly as shown; the values column represents
     *            fields that the user inputs as appropriate.
     * 
        ...
        try
        {
            ...
            for (int i = 0; i < args.length; i++)
            {
                ...
                if (args[i].startsWith("-"))
                {
                    if (args[i].equalsIgnoreCase("-subject"))
                    {
                        if (i + 1 >= args.length)
                        {
                            if (!isTest())
                            {
                                ErrorLog.fatal(this.getClass(), "FATAL ERROR, Missing a value for subject.");
                            }
                            setFatal(true);
                        }
                        else
                    ...
                    else if (args[i].equalsIgnoreCase("-lastModifyTime"))
                    {
                        if (i + 1 >= args.length)
                        {
                            if (!isTest())
                            {
                                ErrorLog.fatal(this.getClass(), "FATAL ERROR, Missing a value for -lastModifyTime.");
                            }
                            setFatal(true);
                        }
                        else
                    ...
                    else if (args[i].equalsIgnoreCase("-interval"))
                    {
                        if (i + 1 >= args.length)
                        {
                            if (!isTest())
                            {
                                ErrorLog.fatal(this.getClass(), "FATAL ERROR, Missing a value for -interval.");
                            }
                            setFatal(true);
                        }
                        else
                ...
                else
                {
                    if (!isTest())
                    {
                        ErrorLog.warn(this.getClass(), "WARNING, BAD INPUT, Ignoring parameter: arg[" + i + "]="
                                + args[i]);
                    }
                }
            }
