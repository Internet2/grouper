...
"${JAVA}" -Xms$MEM_START -Xmx$MEM_MAX $GROUPER_ADD_OPENS -Dgrouper.home="$GROUPER_HOME/" -Dfile.encoding=utf-8 -XX:+UseG1GC -XX:+UseStringDeduplication $GSH_JVMARGS $GROUPER_GSH_JVMARGS -classpath "${GROUPER_CP}" $GSH "$@"
retVal=$?
