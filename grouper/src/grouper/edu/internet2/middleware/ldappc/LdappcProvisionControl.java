...
/**
 * Controls provisioning for the Ldappc application. It determines if Grouper and/or
 * Signet data is to be provisioned and passes the command line input options on to the
 * classes that do the provisioning. The classes that are called will handle obtaining the
 * corresponding type of configuration data.
 */
public class LdappcProvisionControl extends TimerTask {
  ...
  /**
   * The command line input arguments that determine what data is to be provisioned.
   */
  ...
  public LdappcProvisionControl(InputOptions options) {
    ...
    for (String source : configuration.getSourceSubjectHashEstimates().keySet()) {
      DebugLog.info("Estimate(" + source + ") = "
          + configuration.getSourceSubjectHashEstimate(source));
    }
  }

  /**
   * Being the provisioning process. Determine what is to be provisioned base on the input
   * options and call Grouper-specific and/or Signet-specific methods for provisioning.
   */
  public void run() {
    ...
    if (options.getDoGroups() || options.getDoMemberships()) {
      LdappcGrouperProvisioner provisioner = new LdappcGrouperProvisioner(options,
          subjectCache);
