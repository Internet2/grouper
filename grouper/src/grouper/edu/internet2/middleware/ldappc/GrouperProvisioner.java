...
public class GrouperProvisioner extends Provisioner {
   ...
   *           thrown if one or more exceptions occurs that will not stop processing but
   *           should be reported
   */
  public void provision() throws QueryException, SchemaException, NamingException,
      AttributeNotFoundException, SessionException, MultiErrorException, LdappcException {
    //
  ...
  protected void provisionGroups(Set groups) throws NamingException, LdappcException {
    ...
    //
    GroupSynchronizer synchronizer = new GroupEntrySynchronizer(ldapCtx, rootDn,
        configuration, options, getSubjectCache());
  ...
  private Name getSubjectDn(Member member) {
    ...
    try {
      ...
      if ("id".equals(configuration.getSourceSubjectNamingAttribute(member
          .getSubjectSourceId()))) {
        subjectDn = getSubjectCache().findSubjectDn(ldapCtx, configuration,
   ...
   */
  private MembershipSynchronizer getMembershipSynchronizer(String subjectDn)
      throws NamingException {
    //
