 ...
 * @version $Id: StemDisplayNameFilter.java,v 1.3 2008-11-05 16:18:46 shilen Exp $
 */
public class StemDisplayNameFilter extends BaseQueryFilter {
    ...
    throws QueryException
  {
    ...
    Set results;
    if (ns.isRootStem()) {
      results = removeRootStem(GrouperDAOFactory.getFactory().getStem().findAllByApproximateDisplayName(this.val));
    } else {
      results = GrouperDAOFactory.getFactory().getStem().findAllByApproximateDisplayName(this.val, getStringForScope(ns));
    }
    return results;
