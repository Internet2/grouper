...
private String stemId;
   ...
   * @return this for chaining
   */
  private PrivilegeAttributeDefInheritanceFinder assignPrivilegeHelper(Privilege privilege, String searchFor) {
  ...
  public boolean hasAssignedPrivilege() {
    GrouperUtil.assertion(PrivilegeAttributeDefInheritanceFinder.this.privilege != null,  "You must pass the privilege to the finder");
  ...
  public Set<Privilege> findAssignedPrivileges() {
    ...
    return (Set<Privilege>) GrouperSession.internal_callbackRootGrouperSession(new GrouperSessionHandler() {
      ...
      public Object callback(GrouperSession grouperSession) throws GrouperSessionException {
        ...
        if (stem == null && !StringUtils.isBlank(PrivilegeAttributeDefInheritanceFinder.this.stemId)) {
          stem = StemFinder.findByUuid(GrouperSession.staticGrouperSession(), PrivilegeAttributeDefInheritanceFinder.this.stemId, false, new QueryOptions().secondLevelCache(false));
        } 
        if (stem == null && !StringUtils.isBlank(PrivilegeAttributeDefInheritanceFinder.this.stemName)) {
          stem = StemFinder.findByName(GrouperSession.staticGrouperSession(), PrivilegeAttributeDefInheritanceFinder.this.stemName, false, new QueryOptions().secondLevelCache(false));
        }
        ...
        if (subject == null && !StringUtils.isBlank(subjectId) && !StringUtils.isBlank(subjectSourceId)) {
          subject = SubjectFinder.findByIdAndSource(PrivilegeAttributeDefInheritanceFinder.this.subjectId, PrivilegeAttributeDefInheritanceFinder.this.subjectSourceId, false);
        }            
        if (subject == null && !StringUtils.isBlank(subjectId) && StringUtils.isBlank(subjectSourceId)) {
          subject = SubjectFinder.findById(PrivilegeAttributeDefInheritanceFinder.this.subjectId, false);
        }            
        if (subject == null && !StringUtils.isBlank(subjectIdentifier) && !StringUtils.isBlank(subjectSourceId)) {
          subject = SubjectFinder.findByIdentifierAndSource(PrivilegeAttributeDefInheritanceFinder.this.subjectIdentifier, PrivilegeAttributeDefInheritanceFinder.this.subjectSourceId, false);
        }
        if (subject == null && !StringUtils.isBlank(subjectIdentifier) && StringUtils.isBlank(subjectSourceId)) {
          subject = SubjectFinder.findByIdentifier(PrivilegeAttributeDefInheritanceFinder.this.subjectIdentifier, false);
        }
        ...
        for (Stem stemLocal: stems) {
          ...
          stemRuleDefinitions.addAll(GrouperUtil.nonNull(RuleFinder.findAttributeDefPrivilegeInheritRules(stem)));
        }
        ...
        for (RuleDefinition ruleDefinition : GrouperUtil.nonNull(stemRuleDefinitions)) {
          ...
          if (!StringUtils.equals(ruleDefinition.getCheck().getCheckType(), RuleCheckType.attributeDefCreate.name())) {
            continue;
          }
