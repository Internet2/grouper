...
public class Hib3PITAttributeDefNameDAO extends Hib3DAO implements PITAttributeDefNameDAO {
  ...
  public PITAttributeDefName findBySourceIdActive(String id, boolean exceptionIfNotFound) {
    PITAttributeDefName pitAttributeDefName = PITUtils.internal_deleteIfMultipleResults(HibernateSession
      .byHqlStatic()
      ...
      .setString("id", id)
      .list(PITAttributeDefName.class));
  ...
  public long deleteInactiveRecords(Timestamp time) {
    return HibernateSession.byHqlStatic().createQuery(
        "select id from PITAttributeDefName where endTimeDb is not null and endTimeDb < :time").setLong("time", time.getTime() * 1000)
        .deleteInBatches(String.class, "PITAttributeDefName", "id");
