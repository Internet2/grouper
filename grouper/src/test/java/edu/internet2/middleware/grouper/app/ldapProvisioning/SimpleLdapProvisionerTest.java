...
public class SimpleLdapProvisionerTest extends GrouperProvisioningBaseTest {
  ...
  public static void main(String[] args) {
    TestRunner.run(new SimpleLdapProvisionerTest("testProvisioningEntityAttributeResolver2Incremental"));    
  }
  ...
  *
  */
 public void testSimpleLdapEntityProvisionerFullBen() {
   LdapProvisionerTestUtils.configureLdapProvisioner(
       new LdapProvisionerTestConfigInput()
         .assignConfigId("ldapMemberOfProvisioner")
         .assignProvisioningStrategy("benEntityAttributes")
         );
   long started = System.currentTimeMillis();
   ...
   LdapEntry ldapEntry = new LdapEntry("cn=testgroup,ou=Groups,dc=example,dc=edu");
   ldapEntry.addAttribute(new LdapAttribute("cn", "testgroup"));
   ldapEntry.addAttribute(new LdapAttribute("objectClass", "top"));
   ldapEntry.addAttribute(new LdapAttribute("objectClass", "posixGroup"));
   ldapEntry.addAttribute(new LdapAttribute("gidNumber", String.valueOf(testGroup.getIdIndex())));
   LdapSessionUtils.ldapSession().create("personLdap", ldapEntry);

   ldapEntry = new LdapEntry("cn=testgroup3,ou=Groups,dc=example,dc=edu");
   ldapEntry.addAttribute(new LdapAttribute("cn", "testgroup3"));
   ldapEntry.addAttribute(new LdapAttribute("objectClass", "top"));
   ldapEntry.addAttribute(new LdapAttribute("objectClass", "posixGroup"));
   ldapEntry.addAttribute(new LdapAttribute("gidNumber", String.valueOf(testGroup3.getIdIndex())));
   LdapSessionUtils.ldapSession().create("personLdap", ldapEntry);
   
   Subject aanderson = SubjectFinder.findById("aanderson", true);
   Subject ajohnson = SubjectFinder.findById("ajohnson", true);
   ...
   testGroup.addMember(ajohnson, false);

   testGroup3.addMember(ajohnson, false);
   testGroup3.addMember(abrown, false);
   ...
   final GrouperProvisioningAttributeValue attributeValue = new GrouperProvisioningAttributeValue();
   attributeValue.setDirectAssignment(true);
   attributeValue.setDoProvision("ldapMemberOfProvisioner");
   attributeValue.setTargetName("ldapMemberOfProvisioner");
   attributeValue.setStemScopeString("sub");

   GrouperProvisioningService.saveOrUpdateProvisioningAttributes(attributeValue, stem);

   //lets sync these over
   assertEquals(0, LdapSessionUtils.ldapSession().list("personLdap", "ou=People,dc=example,dc=edu", LdapSearchScope.SUBTREE_SCOPE, "(description=*)", new String[] {"uid"}, null).size());

   GrouperProvisioningOutput grouperProvisioningOutput = fullProvision("ldapMemberOfProvisioner");
   GrouperProvisioner grouperProvisioner = GrouperProvisioner.retrieveInternalLastProvisioner();

   assertEquals(0, grouperProvisioningOutput.getRecordsWithErrors());

   List<LdapEntry> ldapEntries = LdapSessionUtils.ldapSession().list("personLdap", "ou=People,dc=example,dc=edu", LdapSearchScope.SUBTREE_SCOPE, "(uid=ajohnson)", new String[] {"description"}, null);
   assertEquals(1, ldapEntries.size());

   ldapEntry = ldapEntries.get(0);

   assertEquals(2, ldapEntry.getAttribute("description").getStringValues().size());
   assertTrue(ldapEntry.getAttribute("description").getStringValues().contains("cn=testgroup,ou=Groups,dc=example,dc=edu"));
   assertTrue(ldapEntry.getAttribute("description").getStringValues().contains("cn=testgroup3,ou=Groups,dc=example,dc=edu"));
   ...
   // try update
   testGroup.deleteMember(aanderson);
   grouperProvisioningOutput = fullProvision("ldapMemberOfProvisioner");
   grouperProvisioner = GrouperProvisioner.retrieveInternalLastProvisioner();
  ...
  public void testProvisioningTypeEntityAttributesDeleteValueManagedByGrouperFull() {
    provisioningTypeEntityAttributesDeleteValueManagedByGrouper(true); 
  }
  
  public void testProvisioningTypeEntityAttributesDeleteValueManagedByGrouperIncremental() {
    provisioningTypeEntityAttributesDeleteValueManagedByGrouper(false); 
  }
  
  public void testProvisioningTypeGroupAttributesDeleteValueManagedByGrouperFull() {
    provisioningTypeGroupAttributesDeleteValueManagedByGrouper(true); 
  }
  
  public void testProvisioningTypeGroupAttributesDeleteValueManagedByGrouperIncremental() {
    provisioningTypeGroupAttributesDeleteValueManagedByGrouper(false); 
  }
  
  /**
   * simple provisioning of subject ids to ldap group
   */
  public void provisioningTypeGroupAttributesDeleteValueManagedByGrouper(boolean isFull) {
    
    LdapAttribute ldapAttribute = new LdapAttribute("member", "uid=aanderson,ou=People,dc=example,dc=edu");
    ldapAttribute.addValue("uid=aanderson727,ou=People,dc=example,dc=edu");
    
    LdapAttribute objectClassAttribute = new LdapAttribute("objectClass", "groupOfNames");
    objectClassAttribute.addValue("top");
    LdapEntry ldapEntry = new LdapEntry("cn=test1:testGroup2,ou=Groups,dc=example,dc=edu");
    ldapEntry.addAttribute(objectClassAttribute);
    ldapEntry.addAttribute(ldapAttribute);
    ldapEntry.addAttribute(new LdapAttribute("businessCategory", "55"));
    LdapSessionUtils.ldapSession().create("personLdap", ldapEntry);
    
    
    ldapAttribute = new LdapAttribute("member", "uid=aanderson,ou=People,dc=example,dc=edu");
    ldapEntry = new LdapEntry("cn=test1:testGroup4,ou=Groups,dc=example,dc=edu");
    
    objectClassAttribute = new LdapAttribute("objectClass", "groupOfNames");
    objectClassAttribute.addValue("top");
    ldapEntry.addAttribute(objectClassAttribute);
    ldapEntry.addAttribute(ldapAttribute);
    ldapEntry.addAttribute(new LdapAttribute("businessCategory", "56"));
    LdapSessionUtils.ldapSession().create("personLdap", ldapEntry);
    
    
    ldapAttribute = new LdapAttribute("member", "uid=aanderson,ou=People,dc=example,dc=edu");
    ldapAttribute.addValue("uid=abrown,ou=People,dc=example,dc=edu");
    ...
    testGroup3.addMember(aanderson, false);
    testGroup3.addMember(abrown, false);
    ...
    assertEquals(3, LdapSessionUtils.ldapSession().list("personLdap", "ou=Groups,dc=example,dc=edu", LdapSearchScope.SUBTREE_SCOPE, "(member=*)", new String[] {"cn"}, null).size());
   
    List<LdapEntry> ldapEntries = LdapSessionUtils.ldapSession().list("personLdap", "ou=Groups,dc=example,dc=edu", LdapSearchScope.SUBTREE_SCOPE, "(cn=test1:testGroup2)", new String[] {"member"}, null);
    assertEquals(1, ldapEntries.size());
    
    ldapEntry = ldapEntries.get(0);
    if (!isFull) {
      assertEquals(2, ldapEntry.getAttribute("member").getStringValues().size());
      assertTrue(ldapEntry.getAttribute("member").getStringValues().contains("uid=aanderson727,ou=People,dc=example,dc=edu"));
    ...
    ldapEntries = LdapSessionUtils.ldapSession().list("personLdap", "ou=Groups,dc=example,dc=edu", LdapSearchScope.SUBTREE_SCOPE, "(cn=test1:testGroup4)", new String[] {"member"}, null);
    assertEquals(1, ldapEntries.size());
    ldapEntry = ldapEntries.get(0);
    if (!isFull) {
    ...
    ldapEntries = LdapSessionUtils.ldapSession().list("personLdap", "ou=Groups,dc=example,dc=edu", LdapSearchScope.SUBTREE_SCOPE, "(cn=test:testGroup3)", new String[] {"member"}, null);
    assertEquals(1, ldapEntries.size());
    ldapEntry = ldapEntries.get(0);
    if (!isFull) {
      assertEquals(2, ldapEntry.getAttribute("member").getStringValues().size());
  ...
  public void testProvisioningEntityAttributeResolverFull() {
    provisioningEntityAttributeResolver(true);
  }
  
  public void testProvisioningEntityAttributeResolverIncremental() {
    provisioningEntityAttributeResolver(false);
  }
  
    
  public void provisioningEntityAttributeResolver(boolean isFull) {
    
    LdapProvisionerTestUtils.configureLdapProvisioner(
        new LdapProvisionerTestConfigInput().assignProvisioningStrategy("configureEntityLinkLdap")
          );
    ...
    List<LdapEntry> ldapEntries = LdapSessionUtils.ldapSession().list("personLdap", "ou=Groups,dc=example,dc=edu", 
        LdapSearchScope.SUBTREE_SCOPE, "(objectClass=groupOfNames)", new String[] {"objectClass", "cn", "member"}, null);
    assertEquals(1, ldapEntries.size());
  ...
  public void testProvisioningEntityAttributeResolver2Full() {
    provisioningEntityAttributeResolver2(true);
  }
  
  public void testProvisioningEntityAttributeResolver2Incremental() {
    provisioningEntityAttributeResolver2(false);
  }
  
    
  public void provisioningEntityAttributeResolver2(boolean isFull) {
    
    LdapProvisionerTestUtils.configureLdapProvisioner(
        new LdapProvisionerTestConfigInput().assignProvisioningStrategy("configureEntityLinkLdap2")
          );
   ...
   * simple provisioning of subject ids to ldap group
   */
  public void provisioningTypeEntityAttributesDeleteValueManagedByGrouper(boolean isFull) {
