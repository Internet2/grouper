 ...
 * will have data to work on. This class assumes the following database structure:
 * 
 * testStem1 teststem2 teststem3 testgroup31 testmember31 testmember32 testmember33
 * testgroup21 testmember21 testmember22 testmember23
 * 
 * @author Gil Singer
 */
public class GroupProcessorTest extends BaseLdappcTestCase {
  ...
  public void testAddMember() {
    ...
    /*
     * The following attempt to check for membership resulted in displaying that the
     * member does not already exist, but the addMember complained that it did already
     * exist. It should not exist and attempts to delete it failed. TODO: Resolve why add
     * member says it already exists.
     * 
     * if (testGroup31.hasMember(groupAsSubject)) { // Remove the member if it already
     * exists in the group; then add it.
     * System.out.println("DEBUG, member already exists.");
     * groupProcessor.deleteMember(testGroup31, groupAsSubject); } else {
   ...
   */
  /*
   * public Member addMember(Group group, Subject subject) { Member member = null; try {
   * // If it already exists, use it. member = StemFinder.findByName(grouperSession,
   * stemFullPath); } catch(MemberNotFoundException mnfe) { // Member does not exist so
   * create it.
   * 
   * member = groupProcessor.addmember(group, subject); if (member == null) {
   * fail("Could not create " + subject.getName()); } } return stem; }
   */
}
