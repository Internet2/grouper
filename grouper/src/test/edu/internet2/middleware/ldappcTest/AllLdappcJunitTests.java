...
public class AllLdappcJunitTests extends TestCase {
  ...
  /**
   * Constant indicating the built-in database is to be used for testing on the biofix
   * system. This is set in the ldappc.properties file and must be consist with it.
   */
  ...
  /**
   * Constant indicating the built-in database is to be used for testing on the Windows
   * system. This is set in the ldappc.properties file and must be consist with it.
   */
  public static final String TEST_CASE_WINDOWS_TEST = "windowsTest";

  /**
   * Constant indicating the Quickstart database is to be used for testing on the Windows
   * system. This is set in the ldappc.properties file and must be consist with it.
   */
  ...
  /**
   * A convenience setting to allow easy switching between running all tests and running a
   * specific (hardwired) test. Set ALL= true to run all tests. When setting ALL = false,
   * change the class in the else section if needed.
   */
  ...
  public static Test suite() {
    ...
    if (ALL) {
      suite = new TestSuite();
      // 20090115 tz suite.addTest(new
      // LdapWrapperTestSetup(AllJUnitQuickStartTests.suite()));
      suite.addTest(new LdapWrapperTestSetup(
          new TestSuite(BushyGroupsProvisionTest.class)));
