...
public class SimpleLdapProvisionerTest extends GrouperProvisioningBaseTest {
  ...
  *
  */
 public void testSimpleLdapEntityProvisionerFullGatech() {
   LdapProvisionerTestUtils.configureLdapProvisioner(
       new LdapProvisionerTestConfigInput()
         .assignConfigId("gted_accounts_gtaccountentitlements")
         .assignProvisioningStrategy("gatechEntitlement")
         );
   long started = System.currentTimeMillis();
   ...
   final GrouperProvisioningAttributeValue attributeValue = new GrouperProvisioningAttributeValue();
   attributeValue.setDirectAssignment(true);
   attributeValue.setDoProvision("gted_accounts_gtaccountentitlements");
   attributeValue.setTargetName("gted_accounts_gtaccountentitlements");
   attributeValue.setStemScopeString("sub");
   ...
   GrouperProvisioningOutput grouperProvisioningOutput = fullProvision("gted_accounts_gtaccountentitlements");
   GrouperProvisioner grouperProvisioner = GrouperProvisioner.retrieveInternalLastProvisioner();
 ...
 public void testSimpleLdapEntityProvisionerFullBen() {
  ...
  public void testSimpleLdapEntityProvisionerFull() {
    ...
    ldapEntry = ldapEntries.get(0);
    String report = GrouperUtil.toStringForLog(ldapEntry.getAttribute("eduPersonEntitlement").getStringValues());
    assertEquals(report, 2, ldapEntry.getAttribute("eduPersonEntitlement").getStringValues().size());
    assertTrue(report, ldapEntry.getAttribute("eduPersonEntitlement").getStringValues().contains("test:testGroup"));
    assertTrue(report, ldapEntry.getAttribute("eduPersonEntitlement").getStringValues().contains("test:testGroup3"));
  ...
  public void provisioningTypeEntityAttributesDeleteValueManagedByGrouper(boolean isFull) {
    ...
    item = new LdapModificationItem(LdapModificationType.ADD_ATTRIBUTE, new LdapAttribute("eduPersonEntitlement", "testGroup4"));
    ldapModificationItems.add(item);
   ...
   * simple provisioning of subject ids to ldap group
   */
  public void testSimpleLdapEntityProvisionerFullSingle() {
    ...
    ldapEntry = ldapEntries.get(0);
    String report = GrouperUtil.toStringForLog(ldapEntry.getAttribute("eduPersonEntitlement").getStringValues());
    assertEquals(report, 0, ldapEntry.getAttribute("eduPersonEntitlement").getStringValues().size());
   ...
   * simple provisioning of subject ids to ldap group
   */
  public void testSimpleLdap20items() {
    ...
    List<String> subjectIds = GrouperUtil.toList("aanderson", "aanderson727", "abrown", "abrown643", 
        "aclark", "adoe", "agasper", "agonazles", "ahenderson", "ahenderson594", "ajohnson", "ajohnson871", 
        "alangenberg", "alangenberg704", "alangenberg855", "alee", "alewis", "amorrison", "amorrison30", "apeterson");
    
    Map<String, Subject> subjectIdToSubject = SubjectFinder.findByIds(subjectIds, "personLdapSource");

    List<Subject> subjects = new ArrayList<Subject>(subjectIdToSubject.values());
    
    for (int i=0 ;i<20;i++) {
      testGroups.add(new GroupSave(this.grouperSession).assignName("test:testGroup" + i).save());
      if (i==0) {
        for (int j=0; j<subjectIds.size(); j++) {
          testGroups.get(i).addMember(subjects.get(j), false);
        }
      } else {
        testGroups.get(i).addMember(subjects.get(i), false);
      }
    }
    ...
    assertEquals(0, grouperProvisioningOutput.getRecordsWithErrors());
    assertEquals(20, LdapSessionUtils.ldapSession().list("personLdap", "ou=Groups,dc=example,dc=edu", LdapSearchScope.SUBTREE_SCOPE, "(objectClass=posixGroup)", new String[] {"objectClass", "cn", "description", "gidNumber"}, null).size());
