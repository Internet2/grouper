 ...
 * @version $Id: TestXml1.java,v 1.2 2006-09-27 13:56:54 blair Exp $
 * @since   1.1.0
 */
public class TestXml1 extends TestCase {

  private static final Log LOG = LogFactory.getLog(TestXml1.class);

  public TestXml1(String name) {
    super(name);
  }
  ...
  public void testFullExportToStringAndFullImportBeneathNewRootStem() {
    LOG.info("testFullExportToStringAndFullImportBeneathNewRootStem");
    try {
      // Export - Setup
      ...
      Stem root = StemFinder.findRootStem(s).addChildStem("new root", "new root");
      XmlImporter     importer  = new XmlImporter(s, new Properties());
      importer.load( root, XmlReader.getDocumentFromString(xml) );
      s.stop();

      // Import - Verify
      s = GrouperSession.start( SubjectFinder.findRootSubject() );
      Stem            ns  = StemFinder.findByName(  s, root.getName() + ":i2" );  
