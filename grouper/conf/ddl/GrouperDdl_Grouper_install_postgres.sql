...
(
    ...
    hibernate_version_number BIGINT,
    subject_identifier0 VARCHAR(1024),
    subject_identifier1 VARCHAR(1024),
    subject_identifier2 VARCHAR(1024),
    id_index BIGINT NOT NULL,
    internal_id BIGINT NOT NULL,
    email0 VARCHAR(255),
...
(
    ...
    member_attr_def_id VARCHAR(40),
    member_group_id VARCHAR(40),
    member_stem_id VARCHAR(40),
    member_id VARCHAR(40) NOT NULL,
    field_id VARCHAR(40) NOT NULL,
...
(
    ...
    owner_attribute_assign_id VARCHAR(40),
    owner_attribute_def_id VARCHAR(40),
    owner_group_id VARCHAR(40),
...
(
    ...
    value_floating DOUBLE PRECISION,
    value_string VARCHAR(4000),
    value_member_id VARCHAR(40),
    PRIMARY KEY (id)
);
...
(
    ...
    id VARCHAR(40) NOT NULL,
    last_updated BIGINT,
    attribute_def_scope_type VARCHAR(32),
    scope_string VARCHAR(1024),
    scope_string2 VARCHAR(1024),
    PRIMARY KEY (id)
);
...
(
    ...
    email VARCHAR(200),
    search_string_lower VARCHAR(4000),
    create_time BIGINT NOT NULL,
    ...
    vetted_email_addresses VARCHAR(4000),
    PRIMARY KEY (uuid)
);
...
(
    ...
    context_id VARCHAR(40),
    job_message_clob varchar(10000000),
    job_message_bytes BIGINT,
    PRIMARY KEY (id)
);
...
(
    ...
    last_full_sync_start TIMESTAMP,
    last_full_sync_run TIMESTAMP,
    last_full_metadata_sync_start TIMESTAMP,
...
(
    ...
    last_updated TIMESTAMP NOT NULL,
    last_group_sync_start TIMESTAMP,
    last_group_sync TIMESTAMP,
    last_group_metadata_sync_start TIMESTAMP,
...
(
    ...
    provisionable_end TIMESTAMP,
    last_updated TIMESTAMP NOT NULL,
    last_user_sync_start TIMESTAMP,
    last_user_sync TIMESTAMP,
    last_user_metadata_sync_start TIMESTAMP,
...
(
    ...
    status VARCHAR(20),
    sync_timestamp_start TIMESTAMP,
    ...
    server VARCHAR(200),
    last_updated TIMESTAMP NOT NULL,
    description_clob VARCHAR(10000000),
...
CREATE INDEX data_row_field_asn_hst1_idx ON grouper_data_row_field_asn_hst (data_row_assign_internal_id, data_field_internal_id, value_dictionary_internal_id);
CREATE INDEX data_row_field_asn_hst2_idx ON grouper_data_row_field_asn_hst (data_row_assign_internal_id, data_field_internal_id, value_integer);
CREATE INDEX data_row_field_asn_hst3_idx ON grouper_data_row_field_asn_hst (data_field_internal_id, value_dictionary_internal_id);
CREATE INDEX data_row_field_asn_hst4_idx ON grouper_data_row_field_asn_hst (data_field_internal_id, value_integer);
...
ALTER TABLE grouper_data_row_field_asn_hst ADD CONSTRAINT data_row_field_asn_hst_fk_3 FOREIGN KEY (data_row_assign_internal_id) REFERENCES grouper_data_row_assign_hst(internal_id);
...
CREATE TABLE grouper_sql_cache_group (
  ...
  last_membership_sync timestamp NULL,
  PRIMARY KEY (internal_id)
);
...
COMMENT ON COLUMN grouper_sync.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';
...
COMMENT ON VIEW grouper_audit_entry_v IS 'Join of audit entry and audit type, and converts member ids to subject ids';
...
COMMENT ON VIEW grouper_change_log_entry_v IS 'Join of change log entry and change log type';
...
COMMENT ON VIEW grouper_composites_v IS 'Grouper_composites_v is a view of composite relationships with friendly names.  A composite is a joining of two groups with a group math operator of: union, intersection, or complement.';
...
COMMENT ON VIEW grouper_ext_subj_v IS 'grouper_ext_subj_v is a view of external subjects, and they attributes';
...
COMMENT ON VIEW grouper_memberships_all_v IS 'Grouper_memberships_all_v holds one record for each immediate, composite and effective membership or privilege in the system for members to groups or stems (for privileges).';
...
COMMENT ON VIEW grouper_pit_memberships_all_v IS 'Grouper_pit_memberships_all_v holds one record for each immediate, composite and effective membership or privilege in the system that currently exists or has existed in the past for members to groups or stems (for privileges).';
...
COMMENT ON VIEW grouper_memberships_lw_v IS 'Grouper_memberships_lw_v unique membership records that can be read from a SQL interface outside of grouper.  Immediate and effective memberships are represented here (distinct)';
...
COMMENT ON VIEW grouper_mship_stem_lw_v IS 'grouper_mship_stem_lw_v unique membership records that can be read from a SQL interface outside of grouper for stems.  Immediate and effective memberships are represented here (distinct)';
...
COMMENT ON VIEW grouper_mship_attrdef_lw_v IS 'grouper_mship_attrdef_lw_v unique membership records of attr defs that can be read from a SQL interface outside of grouper.  Immediate and effective memberships are represented here (distinct)';
...
COMMENT ON VIEW grouper_stems_v IS 'GROUPER_STEMS_V: holds one record for each stem (folder) in grouper, with friendly names';
...
COMMENT ON VIEW grouper_rpt_composites_v IS 'GROUPER_RPT_COMPOSITES_V: report on the three composite types: union, intersection, complement and how many of each exist';
...
COMMENT ON VIEW grouper_rpt_group_field_v IS 'GROUPER_RPT_GROUP_FIELD_V: report on how many unique members are in each group based on field (or list) name and type';
...
COMMENT ON VIEW grouper_rpt_groups_v IS 'GROUPER_RPT_GROUPS_V: report with a line for each group and some counts of immediate and effective members etc';
...
COMMENT ON VIEW grouper_rpt_roles_v IS 'GROUPER_RPT_ROLES_V: report with a line for each role and some counts of immediate and effective members etc';
...
COMMENT ON VIEW grouper_rpt_members_v IS 'GROUPER_RPT_MEMBERS_V: report for each member in grouper_members and some stats like how many groups they are in';
...
COMMENT ON VIEW grouper_rpt_stems_v IS 'GROUPER_RPT_STEMS_V: report with a row for each stem and stats on many groups or members are inside';
...
COMMENT ON VIEW grouper_role_set_v IS 'grouper_role_set_v: shows all role set relationships';
...
COMMENT ON VIEW grouper_attr_def_name_set_v IS 'grouper_attr_def_name_set_v: shows all attribute def name set relationships';
...
COMMENT ON VIEW grouper_attr_assn_action_set_v IS 'grouper_attr_assn_action_set_v: shows all action set relationships';
...
COMMENT ON VIEW grouper_attr_asn_group_v IS 'grouper_attr_asn_group_v: attribute assigned to a group, with related columns';
...
COMMENT ON VIEW grouper_attr_asn_efmship_v IS 'grouper_attr_asn_efmship_v: attribute assigned to an effective membership';
...
COMMENT ON VIEW grouper_attr_asn_stem_v IS 'grouper_attr_asn_stem_v: attribute assigned to a stem and related cols';
...
COMMENT ON VIEW grouper_attr_asn_member_v IS 'grouper_attr_asn_member_v: attribute assigned to a member and related cols';
...
COMMENT ON VIEW grouper_attr_asn_mship_v IS 'grouper_attr_asn_mship_v: attribute assigned to an immediate memberships, and related cols';
...
COMMENT ON VIEW grouper_attr_asn_attrdef_v IS 'grouper_attr_asn_attrdef_v: attribute assigned to an attribute definition, and related columns';
...
COMMENT ON VIEW grouper_attr_asn_asn_group_v IS 'grouper_attr_asn_asn_group_v: attribute assigned to an assignment of attribute to a group, and related cols';
...
COMMENT ON VIEW grouper_attr_asn_asn_efmship_v IS 'grouper_attr_asn_asn_efmship_v: attribute assigned to an assignment of an attribute to an effective membership, and related cols';
...
COMMENT ON VIEW grouper_attr_asn_asn_stem_v IS 'grouper_attr_asn_asn_stem_v: attribute assigned to an assignment of attribute to a stem, and related cols';
...
COMMENT ON VIEW grouper_attr_asn_asn_member_v IS 'grouper_attr_asn_asn_member_v: attribute assigned to an assignment of an attribute to a member, and related cols';
...
COMMENT ON VIEW grouper_attr_asn_asn_mship_v IS 'grouper_attr_asn_asn_mship_v: attribute assigned to an assignment of an attribute to a membership, and related cols';
...
COMMENT ON VIEW grouper_attr_asn_asn_attrdef_v IS 'grouper_attr_asn_asn_attrdef_v: attribute assigned to an assignment of an attribute to an attribute definition, and related cols';
...
COMMENT ON VIEW grouper_aval_asn_group_v IS 'grouper_aval_asn_group_v: attribute assigned to a group with related columns and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_stem_v IS 'grouper_aval_asn_stem_v: attribute assigned to a stem and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_member_v IS 'grouper_aval_asn_member_v: attribute assigned to a member and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_mship_v IS 'grouper_aval_asn_mship_v: attribute assigned to an immediate memberships, and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_attrdef_v IS 'grouper_aval_asn_attrdef_v: attribute assigned to an attribute definition, and related columns and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_asn_group_v IS 'grouper_aval_asn_asn_group_v: attribute assigned to an assignment of attribute to a group, and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_asn_efmship_v IS 'grouper_aval_asn_asn_efmship_v: attribute assigned to an assignment of an attribute to an effective membership, and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_asn_stem_v IS 'grouper_aval_asn_asn_stem_v: attribute assigned to an assignment of attribute to a stem, and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_asn_member_v IS 'grouper_aval_asn_asn_member_v: attribute assigned to an assignment of an attribute to a member, and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_asn_mship_v IS 'grouper_aval_asn_asn_mship_v: attribute assigned to an assignment of an attribute to a membership, and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_aval_asn_asn_attrdef_v IS 'grouper_aval_asn_asn_attrdef_v: attribute assigned to an assignment of an attribute to an attribute definition, and related cols and values (multiple rows if multiple values, no rows if no values)';
...
COMMENT ON VIEW grouper_attr_def_priv_v IS 'grouper_attr_def_priv_v: shows all privileges internal to grouper of attribute defs';
...
COMMENT ON VIEW grouper_perms_assigned_role_v IS 'grouper_perms_assigned_role_v: shows all permissions assigned to roles';
...
COMMENT ON VIEW grouper_perms_role_v IS 'grouper_perms_role_v: shows all permissions assigned to users due to the users being in a role, and the role being assigned the permission';
...
COMMENT ON VIEW grouper_perms_role_subject_v IS 'grouper_perms_role_subject_v: shows all permissions assigned to users directly while in a role';
...
COMMENT ON VIEW grouper_perms_all_v IS 'grouper_perms_all_v: shows all permissions assigned to users directly while in a role, or assigned to roles (and users in the role)';
...
COMMENT ON VIEW grouper_pit_perms_role_v IS 'grouper_pit_perms_role_v: shows all permissions assigned to users due to the users being in a role, and the role being assigned the permission';
...
COMMENT ON VIEW grouper_pit_perms_role_subj_v IS 'grouper_pit_perms_role_subj_v: shows all permissions assigned to users directly while in a role';
...
COMMENT ON VIEW grouper_pit_perms_all_v IS 'grouper_pit_perms_all_v: shows all permissions assigned to users directly while in a role, or assigned to roles (and users in the role)';
...
COMMENT ON VIEW grouper_pit_attr_asn_value_v IS 'grouper_pit_attr_asn_value_v: joins attribute values with their assignments';
...
COMMENT ON VIEW grouper_stem_set_v IS 'grouper_stem_set_v: shows all stem set relationships';
...
COMMENT ON VIEW grouper_ext_subj_invite_v IS 'External subject invites pending, waiting for someone to respond';
...
COMMENT ON VIEW grouper_rules_v IS 'Rules setup in grouper';
...
COMMENT ON VIEW grouper_service_role_v IS 'grouper_service_role_v: shows service admin or user relationships to folders/groups';
...
COMMENT ON TABLE grouper_prov_group IS 'Group mappings in a target system for a provisioner';

COMMENT ON COLUMN grouper_prov_group.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';

COMMENT ON COLUMN grouper_prov_group.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id';

COMMENT ON COLUMN grouper_prov_group.group_internal_id IS 'optional foreign key to grouper_groups.internal_id';

COMMENT ON COLUMN grouper_prov_group.target_group_id IS 'target system group id';

COMMENT ON COLUMN grouper_prov_group.last_updated IS 'timestamp in micros since 1970 when this record was last updated';

COMMENT ON TABLE grouper_prov_group_attr IS 'Provisioner group attributes metadata';

COMMENT ON COLUMN grouper_prov_group_attr.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';

COMMENT ON COLUMN grouper_prov_group_attr.attribute_name IS 'group attribute name e.g. group_name or group_description';

COMMENT ON COLUMN grouper_prov_group_attr.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id; the catalog of group attribute names is scoped per provisioner';

COMMENT ON COLUMN grouper_prov_group_attr.attribute_type IS 'attribute type e.g. string, int, boolean, timestamp';

COMMENT ON COLUMN grouper_prov_group_attr.last_updated IS 'timestamp in micros since 1970 when this record was last updated';

COMMENT ON TABLE grouper_prov_group_attr_value IS 'Provisioner group attribute values';

COMMENT ON COLUMN grouper_prov_group_attr_value.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';

COMMENT ON COLUMN grouper_prov_group_attr_value.prov_group_attr_internal_id IS 'foreign key to grouper_prov_group_attr.internal_id';

COMMENT ON COLUMN grouper_prov_group_attr_value.prov_group_internal_id IS 'foreign key to grouper_prov_group.internal_id';

COMMENT ON COLUMN grouper_prov_group_attr_value.value_integer IS 'integer value used for int, boolean, or timestamp';

COMMENT ON COLUMN grouper_prov_group_attr_value.value_dictionary_internal_id IS 'foreign key to grouper_dictionary.internal_id for string values';

COMMENT ON COLUMN grouper_prov_group_attr_value.last_updated IS 'timestamp in micros since 1970 when this record was last updated';

COMMENT ON TABLE grouper_prov_user IS 'Provisioner users';

COMMENT ON COLUMN grouper_prov_user.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';

COMMENT ON COLUMN grouper_prov_user.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id';

COMMENT ON COLUMN grouper_prov_user.member_internal_id IS 'optional foreign key to grouper_members.internal_id';

COMMENT ON COLUMN grouper_prov_user.target_user_id IS 'target system user id';

COMMENT ON COLUMN grouper_prov_user.last_updated IS 'timestamp in micros since 1970 when this record was last updated';

COMMENT ON TABLE grouper_prov_user_attr IS 'Provisioner user attributes metadata';

COMMENT ON COLUMN grouper_prov_user_attr.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';

COMMENT ON COLUMN grouper_prov_user_attr.attribute_name IS 'user attribute name e.g. user_name or user_email';

COMMENT ON COLUMN grouper_prov_user_attr.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id; the catalog of user attribute names is scoped per provisioner';

COMMENT ON COLUMN grouper_prov_user_attr.attribute_type IS 'attribute type e.g. string, int, boolean, timestamp';

COMMENT ON COLUMN grouper_prov_user_attr.last_updated IS 'timestamp in micros since 1970 when this record was last updated';

COMMENT ON TABLE grouper_prov_user_attr_value IS 'Provisioner user attribute values';

COMMENT ON COLUMN grouper_prov_user_attr_value.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';

COMMENT ON COLUMN grouper_prov_user_attr_value.prov_user_attr_internal_id IS 'foreign key to grouper_prov_user_attr.internal_id';

COMMENT ON COLUMN grouper_prov_user_attr_value.prov_user_internal_id IS 'foreign key to grouper_prov_user.internal_id';

COMMENT ON COLUMN grouper_prov_user_attr_value.value_integer IS 'integer value used for int, boolean, or timestamp';

COMMENT ON COLUMN grouper_prov_user_attr_value.value_dictionary_internal_id IS 'foreign key to grouper_dictionary.internal_id for string values';

COMMENT ON COLUMN grouper_prov_user_attr_value.last_updated IS 'timestamp in micros since 1970 when this record was last updated';

COMMENT ON TABLE grouper_prov_mship_role IS 'Provisioner membership roles';

COMMENT ON COLUMN grouper_prov_mship_role.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';

COMMENT ON COLUMN grouper_prov_mship_role.role_name IS 'membership role name';

COMMENT ON COLUMN grouper_prov_mship_role.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id';

COMMENT ON COLUMN grouper_prov_mship_role.last_updated IS 'timestamp in micros since 1970 when this record was last updated';

COMMENT ON TABLE grouper_prov_mship IS 'Provisioner memberships';

COMMENT ON COLUMN grouper_prov_mship.internal_id IS 'internal integer id for this table.  Do not refer to this outside of Grouper.  This will differ per env (dev/test/prod)';

COMMENT ON COLUMN grouper_prov_mship.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id';

COMMENT ON COLUMN grouper_prov_mship.prov_user_internal_id IS 'foreign key to grouper_prov_user.internal_id';

COMMENT ON COLUMN grouper_prov_mship.prov_group_internal_id IS 'foreign key to grouper_prov_group.internal_id';

COMMENT ON COLUMN grouper_prov_mship.prov_mship_role_internal_id IS 'foreign key to grouper_prov_mship_role.internal_id';

COMMENT ON COLUMN grouper_prov_mship.last_updated IS 'timestamp in micros since 1970 when this record was last updated';

COMMENT ON VIEW grouper_prov_user_attr_v IS 'View of provisioner users joined with their attributes, grouper members, and grouper_sync_member provisioning state. Fans out to one row per user per attribute-value; users with zero attributes appear as one row with null attribute columns';

COMMENT ON COLUMN grouper_prov_user_attr_v.provisioner_name IS 'provisioner name from grouper_sync';

COMMENT ON COLUMN grouper_prov_user_attr_v.subject_source_id IS 'subject source id of the grouper member';

COMMENT ON COLUMN grouper_prov_user_attr_v.subject_id IS 'subject id of the grouper member';

COMMENT ON COLUMN grouper_prov_user_attr_v.subject_identifier0 IS 'first subject identifier of the grouper member';

COMMENT ON COLUMN grouper_prov_user_attr_v.subject_identifier1 IS 'second subject identifier of the grouper member';

COMMENT ON COLUMN grouper_prov_user_attr_v.attribute_name IS 'user attribute name e.g. user_name or user_email';

COMMENT ON COLUMN grouper_prov_user_attr_v.value_string IS 'string attribute value from the grouper dictionary';

COMMENT ON COLUMN grouper_prov_user_attr_v.value_integer IS 'integer attribute value used for int, boolean, or timestamp types';

COMMENT ON COLUMN grouper_prov_user_attr_v.attribute_type IS 'attribute type e.g. string, int, boolean, timestamp';

COMMENT ON COLUMN grouper_prov_user_attr_v.member_name IS 'subject name from grouper_members';

COMMENT ON COLUMN grouper_prov_user_attr_v.member_description IS 'subject description from grouper_members';

COMMENT ON COLUMN grouper_prov_user_attr_v.provisionable IS 'T/F if the grouper member is currently provisionable to this target';

COMMENT ON COLUMN grouper_prov_user_attr_v.in_target IS 'T/F if the grouper member currently exists in the target';

COMMENT ON COLUMN grouper_prov_user_attr_v.in_target_insert_or_exists IS 'T/F: T if grouper inserted the user into the target, F if it already existed';

COMMENT ON COLUMN grouper_prov_user_attr_v.provisionable_start IS 'timestamp when the grouper member became provisionable to this target';

COMMENT ON COLUMN grouper_prov_user_attr_v.provisionable_end IS 'timestamp when the grouper member stopped being provisionable to this target';

COMMENT ON COLUMN grouper_prov_user_attr_v.in_target_start IS 'timestamp when the grouper member first appeared in the target';

COMMENT ON COLUMN grouper_prov_user_attr_v.in_target_end IS 'timestamp when the grouper member was last removed from the target';

COMMENT ON COLUMN grouper_prov_user_attr_v.sync_engine IS 'provisioner engine from grouper_sync';

COMMENT ON COLUMN grouper_prov_user_attr_v.target_user_id IS 'target system user id';

COMMENT ON COLUMN grouper_prov_user_attr_v.grouper_sync_id IS 'grouper_sync uuid';

COMMENT ON COLUMN grouper_prov_user_attr_v.user_last_updated IS 'timestamp in micros since 1970 when the grouper_prov_user row was last updated';

COMMENT ON COLUMN grouper_prov_user_attr_v.value_last_updated IS 'timestamp in micros since 1970 when the grouper_prov_user_attr_value row was last updated';

COMMENT ON COLUMN grouper_prov_user_attr_v.prov_user_internal_id IS 'foreign key to grouper_prov_user.internal_id';

COMMENT ON COLUMN grouper_prov_user_attr_v.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id';

COMMENT ON COLUMN grouper_prov_user_attr_v.member_id IS 'grouper_members.id uuid';

COMMENT ON COLUMN grouper_prov_user_attr_v.member_id_index IS 'grouper_members.id_index integer id';

COMMENT ON COLUMN grouper_prov_user_attr_v.member_internal_id IS 'foreign key to grouper_members.internal_id';

COMMENT ON VIEW grouper_prov_group_attr_v IS 'View of provisioner groups joined with their attributes, grouper_groups, and grouper_sync_group provisioning state. Fans out to one row per group per attribute-value; groups with zero attributes appear as one row with null attribute columns';

COMMENT ON COLUMN grouper_prov_group_attr_v.provisioner_name IS 'provisioner name from grouper_sync';

COMMENT ON COLUMN grouper_prov_group_attr_v.group_name IS 'group system name from grouper_groups';

COMMENT ON COLUMN grouper_prov_group_attr_v.attribute_name IS 'group attribute name';

COMMENT ON COLUMN grouper_prov_group_attr_v.value_string IS 'string attribute value from the grouper dictionary';

COMMENT ON COLUMN grouper_prov_group_attr_v.value_integer IS 'integer attribute value used for int, boolean, or timestamp types';

COMMENT ON COLUMN grouper_prov_group_attr_v.attribute_type IS 'attribute type e.g. string, int, boolean, timestamp';

COMMENT ON COLUMN grouper_prov_group_attr_v.provisionable IS 'T/F if the group is currently provisionable to this target';

COMMENT ON COLUMN grouper_prov_group_attr_v.in_target IS 'T/F if the group currently exists in the target';

COMMENT ON COLUMN grouper_prov_group_attr_v.in_target_insert_or_exists IS 'T/F: T if grouper inserted the group into the target, F if it already existed';

COMMENT ON COLUMN grouper_prov_group_attr_v.provisionable_start IS 'timestamp when the group became provisionable to this target';

COMMENT ON COLUMN grouper_prov_group_attr_v.provisionable_end IS 'timestamp when the group stopped being provisionable to this target';

COMMENT ON COLUMN grouper_prov_group_attr_v.in_target_start IS 'timestamp when the group first appeared in the target';

COMMENT ON COLUMN grouper_prov_group_attr_v.in_target_end IS 'timestamp when the group was last removed from the target';

COMMENT ON COLUMN grouper_prov_group_attr_v.group_description IS 'group description from grouper_groups';

COMMENT ON COLUMN grouper_prov_group_attr_v.group_extension IS 'group extension (last segment of the group name) from grouper_groups';

COMMENT ON COLUMN grouper_prov_group_attr_v.group_display_extension IS 'group display extension (last segment of the display name) from grouper_groups';

COMMENT ON COLUMN grouper_prov_group_attr_v.sync_engine IS 'provisioner engine from grouper_sync';

COMMENT ON COLUMN grouper_prov_group_attr_v.target_group_id IS 'target system group id';

COMMENT ON COLUMN grouper_prov_group_attr_v.grouper_sync_id IS 'grouper_sync uuid';

COMMENT ON COLUMN grouper_prov_group_attr_v.group_last_updated IS 'timestamp in micros since 1970 when the grouper_prov_group row was last updated';

COMMENT ON COLUMN grouper_prov_group_attr_v.value_last_updated IS 'timestamp in micros since 1970 when the grouper_prov_group_attr_value row was last updated';

COMMENT ON COLUMN grouper_prov_group_attr_v.prov_group_internal_id IS 'foreign key to grouper_prov_group.internal_id';

COMMENT ON COLUMN grouper_prov_group_attr_v.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id';

COMMENT ON COLUMN grouper_prov_group_attr_v.group_id IS 'grouper_groups.id uuid';

COMMENT ON COLUMN grouper_prov_group_attr_v.group_id_index IS 'grouper_groups.id_index integer id';

COMMENT ON COLUMN grouper_prov_group_attr_v.group_internal_id IS 'foreign key to grouper_groups.internal_id';

COMMENT ON VIEW grouper_prov_mship_v IS 'View of provisioner memberships joined with provisioner users/groups, grouper_members, grouper_groups, and grouper_sync_membership provisioning state';

COMMENT ON COLUMN grouper_prov_mship_v.provisioner_name IS 'provisioner name from grouper_sync';

COMMENT ON COLUMN grouper_prov_mship_v.subject_source_id IS 'subject source id of the grouper member';

COMMENT ON COLUMN grouper_prov_mship_v.subject_id IS 'subject id of the grouper member';

COMMENT ON COLUMN grouper_prov_mship_v.subject_identifier0 IS 'first subject identifier of the grouper member';

COMMENT ON COLUMN grouper_prov_mship_v.subject_identifier1 IS 'second subject identifier of the grouper member';

COMMENT ON COLUMN grouper_prov_mship_v.member_name IS 'subject name from grouper_members';

COMMENT ON COLUMN grouper_prov_mship_v.group_name IS 'group system name from grouper_groups';

COMMENT ON COLUMN grouper_prov_mship_v.role_name IS 'membership role name from grouper_prov_mship_role';

COMMENT ON COLUMN grouper_prov_mship_v.in_target IS 'T/F if the membership currently exists in the target';

COMMENT ON COLUMN grouper_prov_mship_v.in_target_insert_or_exists IS 'T/F: T if grouper inserted the membership into the target, F if it already existed';

COMMENT ON COLUMN grouper_prov_mship_v.in_target_start IS 'timestamp when the membership first appeared in the target';

COMMENT ON COLUMN grouper_prov_mship_v.in_target_end IS 'timestamp when the membership was last removed from the target';

COMMENT ON COLUMN grouper_prov_mship_v.mship_last_updated IS 'timestamp in micros since 1970 when the grouper_prov_mship row was last updated';

COMMENT ON COLUMN grouper_prov_mship_v.sync_engine IS 'provisioner engine from grouper_sync';

COMMENT ON COLUMN grouper_prov_mship_v.target_user_id IS 'target system user id';

COMMENT ON COLUMN grouper_prov_mship_v.target_group_id IS 'target system group id';

COMMENT ON COLUMN grouper_prov_mship_v.grouper_sync_id IS 'grouper_sync uuid';

COMMENT ON COLUMN grouper_prov_mship_v.grouper_sync_mship_id IS 'target system membership id from grouper_sync_membership.membership_id';

COMMENT ON COLUMN grouper_prov_mship_v.prov_mship_internal_id IS 'foreign key to grouper_prov_mship.internal_id';

COMMENT ON COLUMN grouper_prov_mship_v.prov_user_internal_id IS 'foreign key to grouper_prov_user.internal_id';

COMMENT ON COLUMN grouper_prov_mship_v.prov_group_internal_id IS 'foreign key to grouper_prov_group.internal_id';

COMMENT ON COLUMN grouper_prov_mship_v.grouper_sync_internal_id IS 'foreign key to grouper_sync.internal_id';

COMMENT ON COLUMN grouper_prov_mship_v.member_id IS 'grouper_members.id uuid';

COMMENT ON COLUMN grouper_prov_mship_v.member_id_index IS 'grouper_members.id_index integer id';

COMMENT ON COLUMN grouper_prov_mship_v.member_internal_id IS 'foreign key to grouper_members.internal_id';

COMMENT ON COLUMN grouper_prov_mship_v.group_id IS 'grouper_groups.id uuid';

COMMENT ON COLUMN grouper_prov_mship_v.group_id_index IS 'grouper_groups.id_index integer id';

COMMENT ON COLUMN grouper_prov_mship_v.group_internal_id IS 'foreign key to grouper_groups.internal_id';
...
COMMENT ON TABLE grouper_sql_cache_depend_type IS 'table to store types of dependencies';

COMMENT ON COLUMN grouper_sql_cache_depend_type.internal_id IS 'primary key of the table';

COMMENT ON COLUMN grouper_sql_cache_depend_type.dependency_category IS 'category of dependency type';

COMMENT ON COLUMN grouper_sql_cache_depend_type.name IS 'name of dependency type';

COMMENT ON COLUMN grouper_sql_cache_depend_type.description IS 'description of dependency type';

COMMENT ON TABLE grouper_sql_cache_dependency IS 'table to store dependencies';

COMMENT ON COLUMN grouper_sql_cache_dependency.internal_id IS 'primary key of the table';

COMMENT ON COLUMN grouper_sql_cache_dependency.dep_type_internal_id IS 'foreign key to grouper_sql_cache_depend_type table';

COMMENT ON COLUMN grouper_sql_cache_dependency.owner_internal_id IS 'Something that something else is dependent on.  If something in the owner changes, then the dependent object might need to change';

COMMENT ON COLUMN grouper_sql_cache_dependency.dependent_internal_id IS 'This is the internal id of the dependent object.  Check all the dependent objects if something changes in owner';

COMMENT ON COLUMN grouper_sql_cache_dependency.created_on IS 'when this row was created';

create view grouper_sql_dependency_group_v (dependency_type_name, dependency_type_category, dependency_type_internal_id, dependency_internal_id, owner_cache_group_internal_id, owner_group_name,owner_group_id, owner_group_internal_id, owner_group_id_index, owner_field_name, owner_field_type, owner_field_internal_id, depen_cache_group_internal_id, depen_group_name, depen_group_id, depen_group_internal_id, depen_group_id_index, depen_field_name, depen_field_type, depen_field_internal_id) as select gscdt_group.name as dependency_type_name, gscdt_group.dependency_category as dependency_type_category, gscdt_group.internal_id as dependency_type_internal_id, gscd_group.internal_id as dependency_internal_id, gscg_owner.internal_id as owner_cache_group_internal_id, gg_owner.name as owner_group_name, gg_owner.id as owner_group_id, gg_owner.internal_id as owner_group_internal_id, gg_owner.id_index as owner_group_id_index, gf_owner.name as owner_field_name, gf_owner.type as owner_field_type, gf_owner.internal_id as owner_field_internal_id, gscg_dependent.internal_id as depen_cache_group_internal_id, gg_dependent.name as depen_group_name, gg_dependent.id as depen_group_id, gg_dependent.internal_id as depen_group_internal_id, gg_dependent.id_index as depen_group_id_index, gf_dependent.name as depen_field_name, gf_dependent.type as depen_field_type, gf_dependent.internal_id as depen_field_internal_id from  grouper_sql_cache_depend_type gscdt_group, grouper_sql_cache_dependency gscd_group left join grouper_sql_cache_group gscg_owner on gscd_group.owner_internal_id = gscg_owner.internal_id left join grouper_groups gg_owner on gg_owner.internal_id = gscg_owner.group_internal_id left join grouper_fields gf_owner on gscg_owner.field_internal_id = gf_owner.internal_id  left join grouper_sql_cache_group gscg_dependent on gscd_group.dependent_internal_id = gscg_dependent.internal_id left join grouper_groups gg_dependent on gg_dependent.internal_id = gscg_dependent.group_internal_id left join grouper_fields gf_dependent on gscg_dependent.field_internal_id = gf_dependent.internal_id  where gscdt_group.name in ('mshipHistory_viaAttribute', 'mshipHistory_recentMships','mshipHistory_abac','abac_group')  and gscd_group.dep_type_internal_id = gscdt_group.internal_id ;

COMMENT ON VIEW grouper_sql_dependency_group_v IS 'view on dependencies from group to group.  a group is dependent on another group if when the owner changes the dependent group needs to be recalculated';

COMMENT ON COLUMN grouper_sql_dependency_group_v.dependency_type_name IS 'name of the dependency from the grouper_sql_cache_depend_type table.  e.g. mshipHistory_abac or abac_group';
COMMENT ON COLUMN grouper_sql_dependency_group_v.dependency_type_category IS 'category of the dependency from the grouper_sql_cache_depend_type table.  e.g. mshipHistory or abac';
COMMENT ON COLUMN grouper_sql_dependency_group_v.dependency_type_internal_id IS 'internal id of the dependency type from the grouper_sql_cache_depend_type table';
COMMENT ON COLUMN grouper_sql_dependency_group_v.dependency_internal_id IS 'internal id of the dependency from the grouper_sql_cache_dependency table';
COMMENT ON COLUMN grouper_sql_dependency_group_v.owner_cache_group_internal_id IS 'internal id of the owner of the cache group from the grouper_sql_cache_group table';
COMMENT ON COLUMN grouper_sql_dependency_group_v.owner_group_name IS 'group name of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.owner_group_id IS 'group uuid of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.owner_group_internal_id IS 'group internal id of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.owner_group_id_index IS 'group id index of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.owner_field_name IS 'field name e.g. members of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.owner_field_type IS 'field type e.g. access of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.owner_field_internal_id IS 'field internal id of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.depen_cache_group_internal_id IS 'internal id of the dependent of the cache group from the grouper_sql_cache_group table';
COMMENT ON COLUMN grouper_sql_dependency_group_v.depen_group_name IS 'group name of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.depen_group_id IS 'group uuid of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.depen_group_internal_id IS 'group internal id of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.depen_group_id_index IS 'group id index of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.depen_field_name IS 'field name e.g. members of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.depen_field_type IS 'field type e.g. access of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_group_v.depen_field_internal_id IS 'field internal id of the dependent group of the dependency';

create view grouper_sql_dependency_attr_v (dependency_type_name, dependency_type_category,  dependency_type_internal_id, dependency_internal_id, owner_data_field_internal_id,  owner_data_field_config_id, owner_data_alias_name, owner_data_alias_lower_name,  owner_data_alias_internal_id, depen_cache_group_internal_id, depen_group_name,  depen_group_id, depen_group_internal_id, depen_group_id_index, depen_field_name,  depen_field_type, depen_field_internal_id ) as select gscdt.name as dependency_type_name, gscdt.dependency_category as dependency_type_category, gscdt.internal_id as dependency_type_internal_id, gscd.internal_id as dependency_internal_id, dtf_owner.internal_id as owner_data_field_internal_id, dtf_owner.config_id  as owner_data_field_config_id, gda_owner.name as owner_data_alias_name, gda_owner.lower_name as owner_data_alias_lower_name, gda_owner.internal_id as owner_data_alias_internal_id, gscg_dependent.internal_id as depen_cache_group_internal_id, gg_dependent.name as depen_group_name, gg_dependent.id as depen_group_id, gg_dependent.internal_id as depen_group_internal_id, gg_dependent.id_index as depen_group_id_index, gf_dependent.name as depen_field_name, gf_dependent.type as depen_field_type, gf_dependent.internal_id as depen_field_internal_id from  grouper_sql_cache_depend_type gscdt, grouper_sql_cache_dependency gscd left join grouper_data_field dtf_owner on gscd.owner_internal_id = dtf_owner.internal_id left join grouper_data_alias gda_owner on gda_owner.data_field_internal_id = dtf_owner.internal_id left join grouper_sql_cache_group gscg_dependent on gscd.dependent_internal_id = gscg_dependent.internal_id left join grouper_groups gg_dependent on gg_dependent.internal_id = gscg_dependent.group_internal_id left join grouper_fields gf_dependent on gscg_dependent.field_internal_id = gf_dependent.internal_id  where gscdt.name in ('abac_attribute')  and gscd.dep_type_internal_id = gscdt.internal_id;

COMMENT ON VIEW grouper_sql_dependency_attr_v IS 'view on dependencies with group dependent on an owner data field.  a group is dependent on a data field if when the owner data field value changes the dependent group needs to be recalculated';

COMMENT ON COLUMN grouper_sql_dependency_attr_v.dependency_type_name IS 'name of the dependency from the grouper_sql_cache_depend_type table.  e.g. mshipHistory_abac or abac_group';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.dependency_type_category IS 'category of the dependency from the grouper_sql_cache_depend_type table.  e.g. mshipHistory or abac';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.dependency_type_internal_id IS 'internal id of the dependency type from the grouper_sql_cache_depend_type table';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.dependency_internal_id IS 'internal id of the dependency from the grouper_sql_cache_dependency table';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.owner_data_field_internal_id IS 'internal id of the owner data field from the grouper_data_field table';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.owner_data_field_config_id IS 'config id of the ata field owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.owner_data_alias_name IS 'alias of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.owner_data_alias_lower_name IS 'alias of the data field owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.owner_data_alias_internal_id IS 'internal if of the alias of the data field owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.depen_cache_group_internal_id IS 'internal id of the dependent of the cache group from the grouper_sql_cache_group table';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.depen_group_name IS 'group name of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.depen_group_id IS 'group uuid of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.depen_group_internal_id IS 'group internal id of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.depen_group_id_index IS 'group id index of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.depen_field_name IS 'field name e.g. members of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.depen_field_type IS 'field type e.g. access of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_attr_v.depen_field_internal_id IS 'field internal id of the dependent group of the dependency';
...
COMMENT ON VIEW grouper_sql_dependency_row_v IS 'view on dependencies from group to group.  a group is dependent on another group if when the owner changes the dependent group needs to be recalculated';

COMMENT ON COLUMN grouper_sql_dependency_row_v.dependency_type_name IS 'name of the dependency from the grouper_sql_cache_depend_type table.  e.g. mshipHistory_abac or abac_group';
COMMENT ON COLUMN grouper_sql_dependency_row_v.dependency_type_category IS 'category of the dependency from the grouper_sql_cache_depend_type table.  e.g. mshipHistory or abac';
COMMENT ON COLUMN grouper_sql_dependency_row_v.dependency_type_internal_id IS 'internal id of the dependency type from the grouper_sql_cache_depend_type table';
COMMENT ON COLUMN grouper_sql_dependency_row_v.dependency_internal_id IS 'internal id of the dependency from the grouper_sql_cache_dependency table';
COMMENT ON COLUMN grouper_sql_dependency_row_v.owner_data_row_internal_id IS 'internal id of the owner data row from the grouper_data_row table';
COMMENT ON COLUMN grouper_sql_dependency_row_v.owner_data_row_config_id IS 'config id of the ata row owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_row_v.owner_data_alias_name IS 'alias of the owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_row_v.owner_data_alias_lower_name IS 'alias of the data row owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_row_v.owner_data_alias_internal_id IS 'internal if of the alias of the data row owner of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_row_v.depen_cache_group_internal_id IS 'internal id of the dependent of the cache group from the grouper_sql_cache_group table';
COMMENT ON COLUMN grouper_sql_dependency_row_v.depen_group_name IS 'group name of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_row_v.depen_group_id IS 'group uuid of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_row_v.depen_group_internal_id IS 'group internal id of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_row_v.depen_group_id_index IS 'group id index of the dependent group of the dependency';
COMMENT ON COLUMN grouper_sql_dependency_row_v.depen_field_name IS 'field name e.g. members of the dependent group of the dependency';
