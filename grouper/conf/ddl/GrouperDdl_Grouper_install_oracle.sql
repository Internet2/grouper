...
(
    ...
    hibernate_version_number NUMBER(38),
    subject_identifier0 VARCHAR2(1024),
    subject_identifier1 VARCHAR2(1024),
    subject_identifier2 VARCHAR2(1024),
    id_index NUMBER(38) NOT NULL,
    internal_id NUMBER(38) NOT NULL,
    email0 VARCHAR2(255),
    ...
    search_string0 VARCHAR2(1500),
    search_string1 VARCHAR2(1500),
    search_string2 VARCHAR2(1500),
    search_string3 VARCHAR2(1500),
    search_string4 VARCHAR2(1500),
    name VARCHAR2(2048),
...
(
    flattened_add_timestamp NUMBER(38) NOT NULL,
    member_internal_id NUMBER(38) NOT NULL,
...
COMMENT ON TABLE grouper_prov_user_attr_v IS 'View of provisioner users joined with their attributes, grouper members, and grouper_sync_member provisioning state. Fans out to one row per user per attribute-value; users with zero attributes appear as one row with null attribute columns';
...
COMMENT ON TABLE grouper_prov_group_attr_v IS 'View of provisioner groups joined with their attributes, grouper_groups, and grouper_sync_group provisioning state. Fans out to one row per group per attribute-value; groups with zero attributes appear as one row with null attribute columns';
...
COMMENT ON TABLE grouper_prov_mship_v IS 'View of provisioner memberships joined with provisioner users/groups, grouper_members, grouper_groups, and grouper_sync_membership provisioning state';
...
COMMENT ON table grouper_sql_dependency_group_v IS 'view on dependencies from group to group.  a group is dependent on another group if when the owner changes the dependent group needs to be recalculated';
...
COMMENT ON table grouper_sql_dependency_attr_v IS 'view on dependencies with group dependent on an owner data field.  a group is dependent on a data field if when the owner data field value changes the dependent group needs to be recalculated';
...
COMMENT ON table grouper_sql_dependency_row_v IS 'view on dependencies with group dependent on an owner data row.  a group is dependent on a data row if when the owner data row value changes the dependent group needs to be recalculated';
