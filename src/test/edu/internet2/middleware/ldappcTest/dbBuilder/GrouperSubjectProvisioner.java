...
/**
 * This class provisions Grouper privileged subject data to the LDAP directory for testing
 * purposes.
 */
public class GrouperSubjectProvisioner {
  ...
  /**
   * The root Dn for grouper, useds as a base for the test subjects to be placed into the
   * LDAP directory.
   */
  private String rootDn;

  /**
   * Constructs a <code>GrouperSubjectProvisioner</code> for the given Ldap context.
   * 
   * The following is a sample of the type of ldif file data that corresponds to the data
   * that is provisioned to the LDAP directory by this class that accesses the Grouper
   * database to retrieve subject data.
   * 
   * //dn: uid=iado,ou=people,dc=uob,dc=ac,dc=uk dn:
   * uid=iado,ou=grouperTestSubjects,dc=my-domain,dc=com objectClass: top objectClass:
   * person objectClass: inetOrgPerson cn: Ian Dolphin uid: iado sn: Dolphin
  ...
  public void provision(String testSubjectsName, String testSubjectsRdn)
    ...
    LdapContext ctx = LdapUtil.getLdapContext(ConfigManager.getInstance()
    ...
    if (numberOfSubjects > 0) {
      ...
      while (subjectIterator.hasNext()) {
        subjectCount++;
        /*
         * // Can use the following to limit the number of subjects provisioned for
         * testing. // TEMPORARILY limit to 5 subjects. if (subjectCount > 50) { break; }
         */
        //
        ...
        // example of testSubjectsDn:
        // "ou=testSubjects,ou=testgrouper,dc=my-domain,dc=com";
  ...
  /**
   * This create an DN entry under which to place the test subjects used for testing
   * purposes. It replaces all existing data under the testSubjectOu + rootDn entry.
   * 
   * @param ctx
   *          Ldap context
   ...
   *          Defines the RDN of the entry to be used as the base under which all test
   *          subject directory entries are placed.
   * @param testSubjectsDn
   *          Defines the DN of the entry to be used as the base under which all test
   *          subject directory entries are placed.
   * @throws javax.naming.NamingException
   *           thrown if a Naming exception occurs
   */
  protected void buildSubjectLdapBase(LdapContext ctx, String testSubjectsName,
      String testSubjectsRdn, String testSubjectsDn) throws NamingException {
    ...
    try {
      ...
      ErrorLog.fatal(getClass(), "Program Error, name already bound for: "
          + testSubjectsDn + "\n" + ne.getMessage());
    } catch (NamingException ne) {
      ErrorLog.error(this.getClass(),
          "DEBUG in GrouperSubjectProvisioner.buildSubjectLdapBase, "
              + "naming exception creating subcontext: " + testSubjectsDn + "\n"
              + ne.getMessage());
    }
  ...
  /**
   * This populates the LDAP directory with subject data for testing purposes. It replaces
   * all existing data under the testSubjectOu + rootDn entry from the .
   * 
   * @param ctx
   *          Ldap context
   * @param subject
   ...
   * @param testSubjectsDn
   *          Defines the DN of the entry to be used as the root under which all other
   *          directory entries are placed, as set in the <group> element in the
   *          ldappc.xml file.
   * @throws javax.naming.NamingException
   *           thrown if a Naming exception occurs
   */
  protected void buildSubjectLdapEntry(LdapContext ctx, Subject subject,
      String testSubjectsDn) throws NamingException {
    //
    ...
    // System.out.println("In GrouperSubjectProvisioner.buildSubjectLdapEntry() subjectDn ="
    // + subjectDn);
    ...
    try {
      ...
      ErrorLog.error(this.getClass(),
          "DEBUG in GrouperSubjectProvisioner, naming exception creating subcontext: "
              + subjectDn + "\n" + ne.getMessage());
    }
  ...
  /**
   * Destroy a subcontext. Use LdapUtil.prune to remove subdirectories first.
   * 
   * @param the
   *          subcontext to be removed
   */
  private void destroySubcontext(String subcontext) {
    try {
      ldapCtx.destroySubcontext(subcontext);
    } catch (NameNotFoundException nnfe) {
      ErrorLog.error(getClass(), "Can not destroy subcontext, name not found: "
          + subcontext + "\n" + nnfe.getMessage());
    } catch (NotContextException nce) {
      ErrorLog.error(getClass(), "Can not destroy subcontext, not a context: "
          + subcontext + "\n" + nce.getMessage());
    } catch (ContextNotEmptyException cnee) {
      ErrorLog.error(getClass(), "Can not destroy subcontext, context not empty: "
          + subcontext + "\n" + cnee.getMessage());
    } catch (NamingException ne) {
      ErrorLog.error(getClass(), "Can not destroy subcontext, naming exception: "
          + subcontext + "\n" + ne.getMessage());
    }
  }
