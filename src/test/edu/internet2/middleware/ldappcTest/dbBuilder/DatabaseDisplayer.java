 ...
 * the License is distributed on an "AS IS" BASIS, WITHOU WARRANTIES OR CONDITIONS OF ANY
...
/**
 * Class for displaying a grouper database. This is design specifically to display the
 * testing database will display parts of other databases.
 * 
 * This class should display database containing the following type of structure:
 * 
 * testStem1 teststem2 teststem3 testgroup1 testmember11 testmember12 testmember13
 * testgroup2 testmember21 testmember22 testmember23
 * 
 * @author Gil Singer
 */
public class DatabaseDisplayer {
  ...
  public DatabaseDisplayer() {
    try {
      ...
      ErrorLog.error(this.getClass(),
          "Could not create inital directory context -- naming exception: "
              + ne.getMessage());
    }
  ...
  public void display() {
    ...
    System.out.println("Maximum number of stems to be displayed = " + MAX_STEMS);

    System.out.println("Maximum number of groups to be displayed = " + MAX_GROUPS);

    System.out.println("Maximum number of members to be displayed = " + MAX_MEMBERS);

    System.out.println("Maximum number of persons to be displayed = " + MAX_PERSONS);

    System.out.println("Maximum number of subjects to be displayed = " + MAX_SUBJECTS);
    ...
    String DN_TEST_BASE = ResourceBundleUtil
        .getString(AllLdappcJunitTests.TEST_CONTEXT_BASE);
  ...
  /**
   * Display stem structure starting at the root stem. This method is for development
   * purposes only.
   */
  ...
  /**
   * Display stem structure starting at the specified stem. This method is for development
   * purposes only.
   */
  public void displayStructure(Stem stemBase) {
    ...
    if (stems.size() > 0) {
      ...
      while (it.hasNext()) {
        ...
        stem = (Stem) it.next();
        System.out.println("Stem: " + stem.getExtension() + "  Name: " + stem.getName());
  ...
  public void displayGroupStructure(Group group) {
    ...
    if (members.size() > 0) {
      System.out.println("Group: " + group.getName() + " has " + members.size()
          + " members.");
        ...
        try {
          ...
          if (MAX_SUBJECT_ERRORS != 0) {
            if (subjectErrorCount == MAX_SUBJECT_ERRORS + 1) {
              // ErrorLog.error(this.getClass(),
              // "Limiting number of 'Can not get subject' messages to " +
              // MAX_SUBJECT_ERRORS);
            ...
            if (subjectErrorCount <= MAX_SUBJECT_ERRORS) {
              ErrorLog.error(this.getClass(),
                  "Can not get subject for one or more members.");
  ...
  public void displaySubjects() {
    ...
    System.out.println("Id/TypeID/Name/Source: " + subject.getId() + "   "
        + subject.getType() + "    " + subject.getName() + "    " + subject.getSource());
    ...
    for (Iterator it = subjects.iterator(); it.hasNext();) {
      ...
      if (type.equals("application")) {
        System.out.println("Id/TypeID/Name/SourceId: " + subject.getId() + "   "
            + subject.getType() + "    " + subject.getName() + "    "
      ...
      if (subjectCount > MAX_SUBJECTS) {
        ...
        if (additional > 0) {
          System.out.println(additional
              + " additional subjects not displayed due to Max Subjects reached.");
        }
        break;
      }
    }
   ...
   * 
   * @param dn
   *          the name of the directory context to be displayed
   */
  public void displayBindings(String dn) {
    ...
    try {
      DirContext contextToDisplay = (DirContext) LdapUtil.getLdapContext(
          ConfigManager.getInstance().getLdapContextParameters(), null).lookup(dn);
   ...
   * @return true if success This method is for development purposes only.
   */
  public boolean displayContexts(String context) {
    ...
    if (context == null) {
      ...
      try {
        ...
        ErrorLog.error(this.getClass(), "In displayContexts, destroy failed: "
            + e.getMessage());
        success = false;
      } finally {
        try {
