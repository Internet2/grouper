...
public class QuickStartTestU extends BaseLdappcTestCase {
  ...
  protected void setUp() {
    ...
    //
    try {
      ldapContext = LdapUtil.getLdapContext(ConfigManager.getInstance()
    ...
    try {
      ...
      ErrorLog.fatal(getClass(), "Unable to get provisioned group set :: "
          + e.getMessage());
    }
  ...
  /**
   * Returns the set of provisioned groups. This ASSUMES the ldappc.xml file uses a single
   * stem query of "qsuob"
   */
  ...
  public void testProvisionedGroups() {
    ...
    try {
      ...
      while (groups.hasNext()) {
        /*
         * grouperGroupNames.add(rdnAttr + "=" + LdapUtil.makeLdapNameSafe(((Group)
         * groups.next()) .getName()));
         */
  ...
  public void internalTestProvisionedGroupMappedAttributes() {
    ...
    try {
      ...
      while (groups.hasNext()) {
        ...
        //
        Group group = (Group) groups.next();
        String groupDn = rdnAttr + "=" + LdapUtil.makeLdapNameSafe(group.getName()) + ","
            + grouperGroupRootDn;
        ...
        while (keys.hasNext()) {
          ...
          try {
            String grouperAttrValue = group.getAttributeOrFieldValue(grouperAttr, false,
                true);
            Attribute attribute = attributes.get(ldapAttr);
            if (attribute != null) {
              assertEquals("To many ldap attribute values", 1, attribute.size());
              assertEquals("Grouper and ldap attribute values don't match",
                  grouperAttrValue, attribute.get());
            } else {
        ...
        try {
          ...
          if ("qsuob".equals(source.getId())) {
            ...
            if (listOfMembersInLdap.indexOf(subject.getId()) == -1) {
              fail("**********The subject id of " + subject.getId()
                  + " was not found in " + listOfMembersInLdap);
            }
          }
  ...
  private void provisionAll() {
    String[] args = { "-subject", "GrouperSystem", "-groups", "-memberships",
        "-permissions" };
  ...
  /**
   * Performs a very basic verification that the subjects.ldif file has been loaded.
   */
  private void performVerification() throws Exception {
    ...
    //
    String[] dn = { GROUPER_SUBJECT_ROOT_DN, grouperGroupRootDn, SIGNET_SUBJECT_ROOT_DN };
  ...
  private HashMap hasMemberSearch() {
    ...
    try {
      ...
      NamingEnumeration answer = ldapContext.search(grouperGroupRootDn, matchAttrs,
          attributesToReturn);
      // e.g.: NamingEnumeration answer = ldapContext.search("dc=my-domain,dc=com",
      // matchAttrs);
      if (answer != null) {
        if (!answer.hasMore()) {
      ...
      fail("Could not get search attributes -- naming exception: " + ne.getMessage()
          + "    " + ne.toString());
    }
