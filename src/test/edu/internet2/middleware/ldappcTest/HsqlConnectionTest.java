...
public class HsqlConnectionTest extends BaseLdappcTestCase {
  ...
  public void testHsqlConnection() {
    ...
    try {
      ...
      // conn = DriverManager.getConnection("jdbc:hsqldb:hsql://localhost:9001/mydb",
      // "sa", "");
      // The error:
      // "SQLException getting driver.  Database does not exists in statement [mydb]"
      //
      ...
      //
      /*
       * String testCaseSet = TestOptions.getTestCaseSet(); // Use subject query for all
       * except biofix UC. if ( AllJUnitTests.TEST_CASE_WINDOWS_TEST.equals(testCaseSet)
       * || AllJUnitTests.TEST_CASE_WINDOWS_QS.equals(testCaseSet) ||
       * AllJUnitTests.TEST_CASE_BIOFIX_TEST.equals(testCaseSet) ) { subjectOrIndividual =
       * AllJUnitTests.SUBJECT_QUERY; } else if
       * (AllJUnitTests.TEST_CASE_BIOFIX_UC.equals(testCaseSet) ) { subjectOrIndividual =
       * AllJUnitTests.INDIVIDUAL_QUERY; } else { fail("testCaseSet not recognized: " +
       * testCaseSet); }
       */
