...
public class XmlImporter {
   ...
   * 
   * @param args
   *            args[0] = name of Xml file to process
   * @throws Exception
   */
  public static void main(String[] args) throws Exception {
    ...
    Log log = LogFactory.getLog(XmlImporter.class);
    ...
    if (userImportProperties != null) {
      ...
      try {
        ...
        log.info("Failed to find [" + importProperties
   ...
   * @throws Exception
   */
  public XmlImporter(Properties options) throws Exception {
   ...
   * @throws Exception
   */
  public void load(GrouperSession s, Document doc) throws Exception {
  ...
  public void load(GrouperSession s, Stem rootStem, Document doc)
    ...
    processMetaData(getImmediateElement(root, "metadata"));
    if (isEmpty(importToName)) {
      process(getImmediateElement(root, "data"), NS_ROOT);
    } else {
      process(getImmediateElement(root, "data"), importToName);
    }
    Runtime.getRuntime().gc();
    processMemberships();
    processMembershipLists();
    Runtime.getRuntime().gc();
    processNamingPrivs();
    processNamingPrivLists();
    Runtime.getRuntime().gc();
    processAccessPrivs();
    processAccessPrivLists();
  ...
  public void loadFlatGroupsOrStems(GrouperSession s, Document doc)
    ...
    processMetaData(getImmediateElement(root, "metadata"));
    Element dataE = getImmediateElement(root, "dataList");
    Collection groupElements = getImmediateElements(dataE, "group");
    ...
    Collection stemElements = getImmediateElements(root, "stem");
  ...
  protected void processProperties(Document doc) throws Exception {
    options = safeOptions;
    ...
    if (xmlOptions == null)
      return;
    log
        .info("Merging user supplied options with XML options. Former take precedence");
    options = new Properties(xmlOptions);
    options.putAll(safeOptions);
  }

  protected Properties getImportOptionsFromXml(Document doc) throws Exception {
    log.debug("Attempting to find importOptions in XML");
    Element rootE = doc.getDocumentElement();
    Element importOptionsE = getImmediateElement(rootE, "importOptions");
    if (importOptionsE == null) {
      log.debug("No importOptions tag in XML");
      return null;
    }
    log.debug("Found importOptions tag in XML - loading options");
    Collection options = getImmediateElements(importOptionsE, "options");
    Element optionE;
    Properties props = new Properties();
    Iterator it = options.iterator();
    while (it.hasNext()) {
      optionE = (Element) it.next();
      props.put(optionE.getAttribute("key"), getText(optionE));
   ...
   * @throws Exception
   */
  private void process(Element e, String stem) throws Exception {
    if (e == null)
      return;
    Collection paths = getImmediateElements(e, "path");
    paths.addAll(getImmediateElements(e, "stem"));
  ...
  private void processNamingPrivs() throws Exception {
    Element naming;
    String stem;
    Member member = null;
    Stem grouperStem;
    ...
    for (int i = 0; i < namingPrivs.size(); i++) {
      ...
      if (!isEmpty(group)) {
        ...
        System.out.println("Assigning " + priv + " to "
            + subj.getName() + " for " + stem);
      }

      grouperStem = StemFinder.findByName(s, stem);
      if (!XmlExporter.hasImmediatePrivilege(member.getSubject(),
          grouperStem, priv)) {
        grouperStem.grantPriv(member.getSubject(), Privilege
            .getInstance(priv));
  ...
  private void processMetaData(Element e) throws Exception {
    if (!optionTrue("import.metadata.group-types") || e == null)
      return;
    log.debug("import.metadata.group-types=true - loading group-types");
    Element groupTypesMetaData = getImmediateElement(e,
        "groupTypesMetaData");
    Collection groupTypes = getImmediateElements(groupTypesMetaData,
        "groupTypeDef");
    ...
    while (groupTypesIterator.hasNext()) {
      ...
      fields = getImmediateElements(groupType, "field");
      if (fields.size() > 0)
      ...
      while (fieldsIterator.hasNext()) {
        ...
        if ((isNew || optionTrue("import.metadata.group-type-attributes"))
            && grouperField == null) {
          ...
          if (fieldType.equals("list")) {
            grouperGroupType.addList(s, fieldName, Privilege
                .getInstance(readPriv), Privilege
                .getInstance(writePriv));
          } else if (fieldType.equals("attribute")) {
            grouperGroupType.addAttribute(s, fieldName, Privilege
                .getInstance(readPriv), Privilege
                .getInstance(writePriv), required);
          } else {
  ...
  private void processAccessPrivs() throws Exception {
    Element access;
    String stem;
    Member member = null;
    Group grouperGroup;
    ...
    for (int i = 0; i < accessPrivs.size(); i++) {
      ...
      if (!XmlExporter.hasImmediatePrivilege(
          member.getSubject(), grouperGroup, priv)) {
        grouperGroup.grantPriv(member.getSubject(), Privilege
            .getInstance(priv));
        System.out.println("...assigned");
      } else {
        System.out.println("...already assigned - skiping");
  ...
  private void processAccessPrivLists() throws Exception {
    ...
    for (int i = 0; i < accessPrivLists.size(); i++) {
      ...
      if (!group.equals(lastGroup)) {
        if (!isEmpty(lastGroup)) {
          if (log.isInfoEnabled())
            log.info("Finished loading Access privs for "
                + lastGroup);
        }
      ...
      if (isEmpty(importOption))
        importOption = options.getProperty("import.data.privileges");

      if (isEmpty(importOption) || "ignore".equals(importOption)) {
        if (log.isInfoEnabled())
      ...
      subjects = getImmediateElements(privileges, "subject");
      ...
      while (subjectsIterator.hasNext()) {
        ...
        if (isEmpty(subjectE.getAttribute("immediate")))
        ...
        if ("group".equals(subjectType)) {
          if (subjectIdentifier.startsWith("*")
              && !subjectIdentifier.endsWith("*")) {
            //relative import
            if (!isEmpty(importToName))
              subjectIdentifier = importToName + sep
                  + subjectIdentifier.substring(1);
            else
            ...
            if ("*SELF*".equals(subjectIdentifier))
              ...
              subjectIdentifier = getAbsoluteName(
                  subjectIdentifier, focusGroup
                      .getParentStem().getName());
          }
          try {
            privGroup = GroupFinder
                .findByName(s, subjectIdentifier);
          } catch (Exception e) {
            log.warn("Could not find Group identified by "
                + subjectIdentifier);
            continue;
          }
          ...
          try {
            if (isEmpty(subjectId)) {
              subject = getSubjectByIdentifier(subjectIdentifier,
                  subjectType);
            } else {
              subject = getSubjectById(subjectId, subjectType);
            }
          } catch (Exception e) {
        ...
        if (!XmlExporter.hasImmediatePrivilege( subject,
            focusGroup, privilege)) {
          if (log.isDebugEnabled())
            log.debug("Assigning " + privilege + " to "
                + subject.getName() + " for " + group);
          focusGroup.grantPriv(subject, Privilege
              .getInstance(privilege));
          if (log.isDebugEnabled())
            log.debug("... finished assignment");
        } else {
          if (log.isDebugEnabled())
            log.debug(privilege + " already assigned to "
                + subject.getName() + " so skipping");
        }
      }
    }
    if (log.isInfoEnabled())
  ...
  private void processNamingPrivLists() throws Exception {
    ...
    for (int i = 0; i < namingPrivLists.size(); i++) {
      ...
      if (!stem.equals(lastStem)) {
        if (!isEmpty(lastStem)) {
          if (log.isInfoEnabled())
            log.info("Finished loading Naming privs for "
                + lastStem);
        }
      ...
      while (subjectsIterator.hasNext()) {
        ...
        if ("group".equals(subjectType)) {
          if (subjectIdentifier.startsWith("*")
            ...
            subjectIdentifier = getAbsoluteName(subjectIdentifier,
                stem);
          }
          try {
            privGroup = GroupFinder
            ...
            log.warn("Could not find Stem identified by "
                + subjectIdentifier);
            continue;
          }
        ...
        if (!XmlExporter.hasImmediatePrivilege( subject,
          ...
          if (log.isDebugEnabled())
            log.debug("Assigning " + privilege + " to "
                + subject.getName() + " for " + stem);
          focusStem.grantPriv(subject, Privilege
              .getInstance(privilege));
          if (log.isDebugEnabled())
            log.debug("...assigned");
        } else {
  ...
  private String getAbsoluteName(String name, String stem) {
    if ("*SELF*".equals(name))
      return stem;
  ...
  private void processMemberships() throws Exception {
    Element subject;
    String stem;
    Member member;
    Group group;
    ...
    for (int i = 0; i < memberships.size(); i++) {
      ...
      if (id != null && id.length() != 0) {
        System.out.println("Making " + id + " a member of "
            + group.getName());
        member = MemberFinder.findBySubject(s, SubjectFinder.findById(
            id, "person"));
        ...
        if (groupName != null && groupName.length() != 0) {
          if ("relative".equals(subject.getAttribute("location"))) {
            groupName = group.getParentStem().getName() + sep
                + groupName;
          }
          System.out.println("Making [" + groupName
              + "] a member of " + group.getName());
          Group groupSubj = GroupFinder.findByName(s, groupName);
          member = MemberFinder.findBySubject(s, SubjectFinder
              .findById(groupSubj.getUuid(), "group"));
          if (group != null && !group.hasMember(member.getSubject())) {
            group.addMember(member.getSubject());
            System.out.println("...added");
          } else {
  ...
  private void processMembershipLists() throws Exception {
    ...
    for (int i = 0; i < membershipLists.size(); i++) {
      ...
      if (!groupName.equals(lastGroupName)) {
        if (!isEmpty(lastGroupName)) {
          if (log.isInfoEnabled())
            log.info("Finished loading memberships for "
                + lastGroupName);
        }
      ...
      if (!group.hasType(field.getGroupType())) {
        if (optionTrue("import.data.apply-new-group-types")) {
          if (log.isInfoEnabled())
      ...
      if (!group.canReadField(field)) {
        log.info("No write privilege - ignoring field "
            + field.getName());
        continue;
      }
      ...
      if (!hasComposite && group.getImmediateMembers().size() > 0)
        hasMembers = true;
      Element compE = getImmediateElement(list, "composite");
      ...
      if (compE != null && (!"add".equals(importOption) || hasMembers)) {
        processComposite(compE, group);
        return;
      }
      if (compE != null && hasMembers) {
        log
            .warn("Skipping composite - cannot ad to existing members for "
                + groupName);
        return;
      }
      subjects = getImmediateElements(list, "subject");
      subjectsIterator = subjects.iterator();
      while (subjectsIterator.hasNext()) {
        subjectE = (Element) subjectsIterator.next();
        isImmediate = "true".equals(subjectE.getAttribute("immediate"));
        ...
        if ("group".equals(subjectType)) {
          if (subjectIdentifier.startsWith("*")
            ...
            if ("*SELF*".equals(subjectIdentifier))
              ...
              subjectIdentifier = getAbsoluteName(
                  subjectIdentifier, group.getParentStem()
                      .getName());
          }
          try {
            privGroup = GroupFinder
        ...
        if (!group.hasImmediateMember(subject, field)) {
          if (log.isDebugEnabled())
            log.debug("Making " + subject.getName()
  ...
  private void processComposite(Element composite, Group group)
      throws Exception {
    ...
    try {
      ...
      leftGroup = processGroupRef(leftE, group.getParentStem().getName());
      Element typeE = elements[1];
      compType = processCompositeType(typeE);
      Element rightE = elements[2];
      rightGroup = processGroupRef(rightE, group.getParentStem()
          .getName());
    } catch (Exception e) {
  ...
  private Group processGroupRef(Element groupE, String stem) throws Exception {
    Group group = null;
    ...
    String actualName = getAbsoluteName(name, stem);
    group = GroupFinder.findByName(s, actualName);
    return group;
  }

  private CompositeType processCompositeType(Element typeE) throws Exception {
    ...
    if ("intersection".equals(name)) {
      ...
      throw new IllegalStateException("Invalid CompositeType [" + name
          + "]. union, intersection or complement allowed");
    }
    return type;
  }

  private void processPath(Element e, String stem) throws Exception {
    ...
    processPrivileges(e, newStem.replaceAll(NS_ROOT + sep, ""), "naming");
    processNaming(e, newStem.replaceAll(NS_ROOT + sep, ""));
    process(e, newStem.replaceAll(NS_ROOT + sep, ""));
  }

  private void processGroup(Element e, String stem) throws Exception {
    ...
    try {
      ...
      if ("true".equals(updateAttributes)) {
        if (!isEmpty(displayExtension)
            && !displayExtension.equals(existingGroup
                .getDisplayExtension()))
    ...
    processLists(e, newGroup);
    processPrivileges(e, newGroup, "access");
    processAccess(e, newGroup);
  }

  private void processSubjects(Element e, String stem) throws Exception {
  ...
  private void processLists(Element e, String group) throws Exception {
    Collection lists = getImmediateElements(e, "list");
  ...
  private void processAttributes(Element e, String stem) throws Exception {
    Element groupTypes = getImmediateElement(e, "groupTypes");
    if (groupTypes == null)
      return;
    Collection types = getImmediateElements(groupTypes, "groupType");
    Element groupType;
    ...
    while (typesIterator.hasNext()) {
      ...
      if (!group.hasType(grouperGroupType)) {
        if (optionTrue("import.metadata.apply-new-group-types")) {
          group.addType(grouperGroupType);
        } else {
          continue;
        }
      }
      attributes = getImmediateElements(groupType, "attribute");
      ...
      while (attributesIterator.hasNext()) {
        ...
        if (!group.canWriteField(field)) {
          log.info("No write privilege. Attribute [" + name
              + "] for [" + group.getName() + "] ignored");
          continue;
        }
        ...
        if (value != null
            ...
            && (isEmpty(origValue) || optionTrue("import.data.update-attributes")))
          group.setAttribute(name, value);
      }
    }
  }

  private void processAccess(Element e, String stem) throws Exception {
  ...
  private void processNaming(Element e, String stem) throws Exception {
  ...
  private void processPrivileges(Element e, String stem, String type)
      throws Exception {
    Collection privileges = getImmediateElements(e, "privileges");
  ...
  private String joinStem(String stem, String extension) {
   ...
   */
  private Collection getImmediateElements(Element element, String elementName)
      throws Exception {
   ...
   */
  private Element getImmediateElement(Element element, String elementName)
      throws Exception {
    NodeList nl = element.getElementsByTagName(elementName);
    Collection elements = new Vector();
    if (nl.getLength() < 1) {
    ...
    if (nl.getLength() > 1) {
      throw new IllegalArgumentException(elementName
          + " occurs more than once - should only occur once");
    }
  ...
  private boolean optionEquals(String key, String value) {
  ...
  private boolean optionDefaultEquals(String key, String value, String value1) {
  ...
  private void processStem(Element e) throws Exception {
    String extension = e.getAttribute("extension");
    String displayExtension = e.getAttribute("displayExtension");
    Element descE = getImmediateElement(e, "description");
    String description = "";

    if (descE != null)
      description = getText(descE);
    ...
    try {
      ...
      log.error("Cannot find stem identified by id=" + id + " or name="
          + name);
      return;
    }

    processPrivileges(e, existingStem.getName(), "naming");
  }

  private void processGroup(Element e) throws Exception {
    String extension = e.getAttribute("extension");
    String displayExtension = e.getAttribute("displayExtension");
    Element descE = getImmediateElement(e, "description");
    String description = "";
    ...
    try {
      ...
      if ("true".equals(updateAttributes)) {
        ...
        if (!isEmpty(description)
            && !description.equals(existingGroup.getDescription()))
          existingGroup.setDescription(description);
        processAttributes(e, existingGroup.getName());
      }

    } catch (GroupNotFoundException ex) {
      log.error("Cannot find Group identified by id=" + id + " or name:"
          + name);
      return;
    }

    processLists(e, existingGroup.getName());
    processPrivileges(e, existingGroup.getName(), "access");
  }
   ...
   *  
   */
  private static String getText(Element element) throws Exception {

    element.normalize();
    NodeList nl = element.getChildNodes();
    if (nl.getLength() != 1) {
      throw (new Exception("Cannot process " + element.getTagName()
          + " tag"));
    }
    Node n = nl.item(0);
  ...
  /**
   * @param options
   *            The options to set. These options supplement and override
   *            those defined in the XML.
   */
  ...
  public static void commandLineUsage() {
    ...
    System.out
        .println("args: subjectIdentifier [(-id <id> | -name <name> | -list)]");
    ...
    System.out
        .println("  -id,               The Uuid of a Stem, into which, data will be");
    System.out.println("                     imported*");
    System.out
        .println("  -name,             The name of a Stem, into which, data will be");
    System.out.println("                     imported*");
    System.out
        .println("                     *If no -id / -name is specified, use=ROOT stem");
    System.out
        .println("  -list,             File contains a flat list of Stems or Groups");
    System.out
        .println("                     which may be updated. Missing Stems and Groups");
    ...
    System.out
        .println("                     which configures the import. Check Javadoc for");
    System.out
        .println("                     a list of properties. If 'properties' is not ");
    System.out
        .println("                     specified, XmlImporter will look for ");
    System.out
        .println("                     'import.properties' in the working directory. ");
    System.out
        .println("                     If this file does not exist XmlImporter will ");
    System.out
        .println("                     look on the classpath. If 'properties' is not ");
    System.out
        .println("                     specified and 'import.properties' cannot be ");
    System.out
        .println("                     found and import options are not included in ");
    System.out
        .println("                     the XML, the import will fail.");
    System.out.println();
  }
}
