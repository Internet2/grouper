...
public class GroupStringMembershipSynchronizer extends GrouperSynchronizer {
  ...
  protected void populate() throws NamingException {
    ...
    for (Map.Entry<String, LdapSearchFilter> entry : getConfiguration()
      ...
      // filter with "*"
      //
      String ldapFilter = LdapUtil.convertParameterToAsterisk(ldapSearchFilter
          .getFilter(), 0);
      ...
      try {
        answer = getContext().search(ldapSearchFilter.getBase(), filterExpr,
            searchControls);
      } catch (Exception e) {
   ...
   */
  protected void commit(Vector<Exception> caughtExceptions) throws NamingException,
      LdappcException {
  ...
  public void synchronize(Set<String> groups) throws NamingException, LdappcException {
    ...
    if (caughtExceptions.size() > 0) {
      throw new MultiErrorException(
          "Non-fatal errors occurred processing the members for " + groupNameString,
