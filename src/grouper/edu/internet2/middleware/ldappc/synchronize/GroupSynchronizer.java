...
public abstract class GroupSynchronizer extends GrouperSynchronizer {
   ...
   */
  protected abstract void performInclude(Group group, int status) throws NamingException,
      LdappcException;
  ...
  public void synchronize(Set groups) throws NamingException, LdappcException {
    ...
    while (grpIterator.hasNext()) {
      ...
      try {
        ...
        ErrorLog.error(getClass(), "GROUP[" + GrouperProvisioner.getGroupData(group)
            + "] :: " + e.toString());
    ...
    if (caughtExceptions.size() > 0) {
      throw new MultiErrorException(
          "Non-fatal errors occurred processing the groups for " + getRoot(),
