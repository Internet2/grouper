 ...
 * @author mchyzer
 */
public class WsSampleGetGroupsJsonpRest implements WsSampleRest {
  ...
  public static void getGroupsLite(WsSampleRestType wsSampleRestType) {
    ...
    if (wsSampleRestType != WsSampleRestType.json) {
      throw new RuntimeException("must be json");
    }
    ...
    try {
      ...
      PostMethod method = new PostMethod(
          RestClientSettings.URL + "/" + RestClientSettings.VERSION  
            + "/subjects" + (wrapResponse ? "?grouperJsonResponseWrapper=something" : ""));
      ...
      subjectLookups[0] = new WsSubjectLookup("test.subject.0", null, null);

      subjectLookups[1] = new WsSubjectLookup("test.subject.1", null, null);
      ...
      response = StringUtils.defaultString(response).trim();
      
      //lets strip out the function call
      if (wrapResponse) {
        if (!response.startsWith("something(") || !response.endsWith(")")) {
          throw new RuntimeException("Why not wrapped?");
        }
        response = response.substring("something(".length(), response.length()-1);
      }
